...
[NOTE]
====
For a complete, working example you can use as a starting point for your own agent network project, see the IT Broker example [link tk]. The example walks through a fully configured `agent-network.yaml`, `exchange.json`, and `/agent-script` directory that you can adapt to your own brokers, agents, MCP servers, and LLMs.
====
