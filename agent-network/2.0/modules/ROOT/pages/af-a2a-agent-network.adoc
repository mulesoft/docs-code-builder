...
Agent network brokers support serving streaming responses to clients using Server-Sent Events (SSE). A client can call a broker with streaming enabled and receive incremental updates as the broker processes the request.

However, brokers don't use streaming when communicating with downstream brokers or agents. Outbound requests from a broker to other brokers or agents use standard non-streaming communication.

[[concurrent-request-handling]]
== How Agent Brokers Handle Concurrent Requests
