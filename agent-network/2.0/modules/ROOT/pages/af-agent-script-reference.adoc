...
NOTE: Agent network only supports triggers using JSON-RPC or HTTP JSON transport.
...
Use this table to decide which node type fits your use case.
...
|*Calls actions (A2A/MCP)*
|Yes -- LLM chooses when and how
|Yes -- LLM coordinates multiple agents/tools
|No
|Yes -- deterministic, fully-bound arguments

|*Number of LLM iterations*
|Multiple (configurable loop)
|Multiple (configurable loop)
|Exactly one
|Zero
...
|*Best for*
|Single-task reasoning: classification, extraction, semantic routing, human-in-the-loop decisions
|Multi-agent coordination: delegating to several agents/tools to achieve a complex goal
|One-shot text generation: summaries, formatting, templated responses
|Deterministic execution: calling actions with known arguments, setting variables, no LLM judgment needed

|===

==== Orchestrator vs. Subagent: When to Use Which

Both the orchestrator and subagent nodes run an LLM reasoning loop with access to actions. The key differences are:

* *Orchestrator* is optimized for workflows where the LLM must coordinate multiple external agents and tools to achieve a goal. The LLM decides which agents to call, in what order, and how to combine their results. Use it when the node needs access to two or more actions and the sequencing between them requires judgment.

* *Subagent* is a general-purpose reasoning node ideal for single-task work: classifying input, extracting structured data, making a routing decision, or interacting with one tool. It also supports human-in-the-loop patterns where the LLM can request clarification.

*Rule of thumb:* If the node coordinates multiple agents or tools toward a shared goal, use `orchestrator`. If it reasons about a single task (even if it uses one action), use `subagent`.
...
The executor node executes Agent Script statements deterministically -- no LLM is involved. Because there is no LLM reasoning loop, every action argument must be fully resolved at execution time (you cannot use slot filling `...` in an executor). Use this node when you need to call an action with known arguments, store a computed value in a variable, or chain multiple deterministic steps before transitioning.
...
[source,yaml]
----
executor prepareAndNotify:
  description: "Sets the ticket status variable and sends a Slack notification."
  do: ->
    set @variables.ticketStatus = "resolved"
    run @actions.send_slack_message
      with text="Ticket " + @generator.classifySeverity.output.ticket_id + " resolved"
      with channel_id=@variables.notificationChannel
  on_exit: -> transition to @echo.finalResponse
----
...
|`do`
|A procedure containing one or more Agent Script statements to execute deterministically. See <<executor-supported-statements>>.
|Procedure
|Yes

|===

[[executor-supported-statements]]
==== Supported Statements in `do`

The `do` block accepts a procedure (`->`) containing one or more of these statements:
...
|`run @actions.<name>`
|Invoke a defined action. Use `with` to bind arguments from variables, node outputs, or literal values. All arguments must be fully resolved (no slot filling). Multiple `with` clauses can be chained.
|`run @actions.escalate` or `run @actions.send_slack_message with text=@generator.summary.output with channel_id="my-channel"`

|`set @variables.<name> = <expression>`
|Store a value in a workflow variable for use in downstream nodes.
|`set @variables.ticketStatus = "escalated"`

|===

==== Executor Node Constraints

Because the executor has no LLM, these constraints apply:

* *No slot filling* -- You cannot use `...` to let an LLM decide an argument value. Every value must come from a literal, a `@variables` reference, a `@request` reference, or a previous node's `.output`.
* *No reasoning loop* -- The `do` block runs exactly once. There is no `max_number_of_loops` or retry behavior.
* *No `outputs` schema* -- The executor does not produce structured LLM output. Downstream nodes access the action's raw response if needed.
* *No `system.instructions`* -- Not applicable since no LLM is called.
