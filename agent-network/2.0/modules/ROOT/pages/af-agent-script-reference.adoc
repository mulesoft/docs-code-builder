...
Both the orchestrator and subagent nodes run an LLM reasoning loop with access to actions. These are the key differences.
...
TIP: If the node coordinates multiple agents or tools toward a shared goal, use `orchestrator`. If it reasons about a single task (even if it uses one action), use `subagent`.
...
The executor node executes Agent Script statements deterministically and no LLM is involved. Because there is no LLM reasoning loop, every action argument must be fully resolved at execution time (you cannot use slot filling `...` in an executor). Use this node when you need to call an action with known arguments, store a computed value in a variable, or chain multiple deterministic steps before transitioning.
