...
Brokers in Agent Network 2.0 architecture leverage a guided deterministic approach by separating steps requiring probabilistic LLM reasoning from those requiring deterministic control-flow logic.
...
* LLM Proxy:
+
xref:gateway::flex-gateway-llm-proxy.adoc[Omni Gateway LLM Proxy] provides a unified endpoint for multiple LLM providers including Gemini, OpenAI, Bedrock (Anthropic Claude models), and NVIDIA Nemotron. However, agent networks only supports direct access to OpenAI and Gemini through LLM Proxy. Transcoding other models to OpenAI format to use in agent networks is also not supported.
