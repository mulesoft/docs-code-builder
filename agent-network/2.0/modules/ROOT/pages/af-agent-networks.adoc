...
[[a2a-protocol]]
== A2A Protocol in Agent Networks

The Agent2Agent (A2A) Protocol governs agent-to-agent communication and powers orchestration, observability, and governance features in agent networks. For more information, see xref:af-a2a-agent-network.adoc[].

NOTE: With A2A v1.0, agent networks support streaming with Server-Sent Events (SSE), both for calling A2A agents that support streaming and for streaming as an A2A endpoint itself.
