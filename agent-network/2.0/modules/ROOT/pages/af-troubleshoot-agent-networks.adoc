...
If the broker returns *401 Unauthorized* error when it routes a request to a downstream A2A agent, the A2A protocol version declared for that agent in `agent-network.yaml` most likely doesn't match the agent's actual
runtime protocol version.

When the declared version does not match the real version, the platform applies the wrong A2A Agent Card policy. That policy does not rewrite the agent card's `url` field to the egress proxy URL, so the A2A SDK posts `send_message`
