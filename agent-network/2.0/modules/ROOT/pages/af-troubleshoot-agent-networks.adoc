...
=== Issue: Linting Errors in Agent Script File

If you see linting errors in your `.agent` file in VS Code, you likely have the Agentforce DX extension pack installed, and it's conflicting with Anypoint Code Builder. This extension pack includes a dependency on the Agent Script extension that conflicts with the Agent Script extension built into Anypoint Code Builder. 

To resolve this conflict, uninstall the Agentforce DX extension pack from VS Code and restart the IDE. Anypoint Code Builder already includes all Agent Script functionality, so you don't need a separate extension.
...
[[asset-belongs-to-another-project-error-2006]]
=== Error: "Asset belongs to another project" (Error 2006)

If the publish command fails with this error (code `2006`), an asset in your project has the same name as an asset that belongs to a different project in the same organization. Asset names must be unique across all projects in an org, and you can't reuse an asset name that another project already owns.

To resolve this conflict:

* In `agent-network.yaml`, rename the conflicting asset to a unique name.
* Check the other project referenced in the error to verify which assets it owns. If you own the conflicting project, remove or rename the asset in that project before you republish.
...
If the broker returns *401 Unauthorized* error when it routes a request to a downstream A2A agent, the A2A protocol version declared for that agent in `agent-network.yaml` might not match the agent's actual
runtime protocol version.

When the declared version doesn't match the real version, the platform applies the wrong A2A Agent Card policy. That policy doesn't rewrite the agent card's `url` field to the egress proxy URL, so the A2A SDK posts `send_message`
directly to the external gateway URL and bypasses the egress proxy. Because the egress proxy is what injects the `client_id` and `client_secret` headers required by the Client-ID-Enforcement policy, the request arrives without
credentials and is rejected with a `401` error.

To resolve this error:

. Confirm the agent's real A2A protocol version. Request the agent card in a browser or Postman by appending `.well-known/agent-card.json` to the connection URL, and check the `protocolVersion` field.
. Update the connection definition in `agent-network.yaml` so the declared protocol version matches the agent's real version.
. Rebuild, republish, and redeploy the agent network.

NOTE: A protocol version mismatch surfaces as an authentication (401) error, not as a version error. Verify the protocol version whenever a downstream agent call fails with `401` even if you've correctly configured the the egress auth policy.
