...
If your broker deploys successfully and answers general questions, but fails whenever it must route a request to a downstream agent or MCP server, check if *Last-Mile Security* is enabled on the Omni Gateway used for outbound
(egress) traffic.
...
* If Last-Mile Security is required for other applications on that gateway, deploy your agent network to a separate egress gateway that has it disabled.
