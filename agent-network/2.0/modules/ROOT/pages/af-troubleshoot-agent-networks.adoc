...
. For `INSECURE-LOGGING` set the log level to `DEBUG`.
...
If the broker responds with information from the LLM's training data instead of delegating to your agent or calling an MCP tool, the LLM's instructions aren't explicit enough about delegation. This is especially common with Gemini on widely-known topics.
...
=== Issue: Broker Returns Classifier Output Instead of Calling MCP Tool

If the broker returns the classifier's output (for example, `{"intent":"get_weather","city":"London"}`) as the final response when the task state requires input, the MCP tool was never invoked. The `classify -> router -> executor` pattern can fail for MCP tool calls.

Instead, try the direct pattern: `trigger -> executor (MCP tool) -> generator -> echo`

Reserve the `classify -> router` pattern for multi-agent routing scenarios.
...
. The A2A agent is not configured to use A2A protocol version 1.0.0. In this case:
+
