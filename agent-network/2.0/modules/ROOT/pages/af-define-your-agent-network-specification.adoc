...
+
.. Set `kind` to `llm`.
.. Set `ref.name` to the `assetId` you copied from `exchange.json`.
...
+
You do not need to add the LLM Proxy to the `registry` section of `agent-network.yaml`.
...
When you deploy your agent network, you need to provide an `apiKey` to authenticate with the LLM Proxy. LLM Proxy authenticates with a `client_id` and `client_secret`. Provide the value in the following format:
