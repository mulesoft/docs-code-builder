...
[IMPORTANT]
Asset names must be unique across all projects in your organization. If another project already uses an asset name, the publish command fails. Use unique, descriptive names for each asset in `agent-network.yaml`. For more information, see xref:af-troubleshoot-agent-networks.adoc#asset-belongs-to-another-project-error-2006[Troubleshoot Agent Networks].
...
The `apiVersion` field in `exchange.json` is the version group shown when you publish an agent network to Exchange. The `apiVersion` field has these behaviors:

* The version group name only changes when you explicitly rename `apiVersion`. Changing `apiVersion` automatically increments the major version (`x.0.0`). 
* The version group name doesn't need to follow a `v#` pattern. You can can use any string.
* Any other change unrelated to `apiVersion` automatically increments only the patch version.
* You can't manually increment the minor version. Exchange handles minor version increments. 
