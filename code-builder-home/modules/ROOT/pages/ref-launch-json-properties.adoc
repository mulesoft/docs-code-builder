...
| `mule.runtime.args`
| No
| Command-line arguments passed to the Mule runtime JVM. Use the `-M-D` prefix for system properties and the `-M-X` prefix for JVM tuning flags. +
 +
Examples: +
`-M-Dencryption.key=myKey` (encryption key) +
`-M-Denv=dev` (environment selector) +
`-M-Dhttp.port=8082` (HTTP listener port) +
`-M-Xmx2g` (maximum JVM heap size) +
`-M-Xms512m -M-Xmx2g` (initial and maximum JVM heap size) +
 +
