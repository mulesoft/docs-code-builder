...
+
When you step into a flow reference, Anypoint Code Builder opens the Mule configuration file that declares that referenced flow, moves the canvas to that flow, and pauses on the first processor inside it. If the referenced flow is in an imported Mule project dependency, the dependency flow opens in a read-only canvas and XML view. You can step through its processors, and stepping continues in the caller flow where appropriate after you step through the referenced flow.
+
.Step Into Details
[%collapsible]
====
...
. Ensure your Mule application is running.
