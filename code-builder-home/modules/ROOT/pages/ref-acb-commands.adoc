...
* Log in to Anypoint Platform from. See xref:start-acb.adoc#login-ide[Log in through the Command Palette].
+
[source,command]
----
...
* Open the *Settings* page for Anypoint Code Builder settings, which include settings for the Anypoint Platform host (EU or US). For an example, see xref:start-acb.adoc#change-clouds[Set Anypoint Code Builder to the EU or US Cloud].
+
[source,command]
----
...
* Import a JAR file for a Mule project and load it into a workspace. See xref:int-import-mule-project.adoc[].
+
[source,command]
----
...
* Scaffold a local API spec project into an implementation project. See xref:imp-implement-local-apis.adoc[Scaffold an API Spec into a New Implementation Project].
+
[source,command]
----
...
----
+
NOTE: This command is available only for API specifications that haven't been published to Anypoint Exchange. If you publish the spec to Exchange, re-scaffolding is no longer available for that asset.
...
* Open the Anypoint Code Builder panel. The panel provides links to frequent actions. See an example in xref:start-discover-ui.adoc[].
+
[source,command]
----
