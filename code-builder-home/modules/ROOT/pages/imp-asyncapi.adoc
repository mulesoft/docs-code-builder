...
:supported-spec-version: Only supported versions of the AsyncAPI specifications in Exchange are available for scaffolding. Unsupported versions don't appear in the search results.

[[intro]]
Anypoint Code Builder supports the implementation of AsyncAPI 2.6 and 2.0 specifications in projects running on Mule runtime version 4.6 or later. When creating an implementation project, Anypoint Code Builder imports and scaffolds an AsyncAPI specification hosted on Anypoint Exchange into an implementation project that you can develop further according to your business needs. 
...
* If you don't yet have access to Anypoint Code Builder, follow the procedures in xref:start-acb.adoc[].
...
+
//note on publishing and designing asyncapi specs from Design Center / MuleSoft VCS
...
* Considers one specification at a time
* Treats the imported specification as a modification of an existing specification and evaluates it for rescaffolding
...
+
** *Subscribe* (`<apikit-asyncapi:publish/>`): AsyncAPI *subscribe* operations in the specification are available for configuration as AsyncAPI *Publish* operations. See the <<ex-flows-implemented, implemented example>> in Example: Scaffolded Flows.
** *Message Listener* (`<apikit-asyncapi:message-listener/>`): AsyncAPI *publish* operations in the specification are configurable as *Message Listener* operations. The system publishes messages to a channel, and the Mule application listens for these messages. See examples in <<ex-flows-scaffolded>>.
+
...
[[scaffold-procedures]]
== Scaffold an AsyncAPI Specification into a Mule Project
...
[IMPORTANT]
====
When re-scaffolding, the scaffolder acts on new combinations of channel, server, and *publish* and *subscribe* operations groupings in the specification and doesn’t re-scaffold combinations that aren’t new.
====
...
+
image::imp-asyncapi-project-ex.png["AsyncAPI implementation project"]
+
...
. Meet the prerequisites in <<prereqs>>.
+
Use of AsyncAPI features requires enablement by an Anypoint Platform organization administrator.
...
My AsyncAPI Spec
----
. Wait for Anypoint Code Builder to load a list of matches to the name, and then select your AsyncAPI specification.
+
{supported-spec-version}
...
+
When the scaffolding process is complete, Anypoint Code Builder opens to a configuration XML file that contains the scaffolded flows (`flows.xml`). If you re-scaffolded the specification, see the <<important-rescaffold, important note>> about which new configurations are scaffolded into your project.
// Shared step on adding business logic
include::partial$acb-reusable-steps.adoc[tags="implement-business-logic"]

== Deploy an Implementation

Deploy the implementation to CloudHub and CloudHub 2.0 or to another host. You can deploy to CloudHub or CloudHub 2.0 from xref:int-deploy-mule-apps.adoc[Anypoint Code Builder]. For other deployment options, see xref:runtime-manager::deployment-strategies.adoc[].
...
+
You can discover the URL for an MQ broker from the region that you select when creating the queue. See the *copy* button in xref:mq::mq-queues.adoc#create-a-queue[Create a Queue], and see xref:mq::mq-client-apps.adoc[].
...
When scaffolding the specification into an implementation project, Anypoint Code Builder creates a flow (`<flow/>`) for each *publish* operation and transforms the *publish* operation into a *Message Listener* source from the AsyncAPI module for APIkit (`<apikit-asyncapi:message-listener/>`). Each flow also includes a *Logger* component. For more information about the module, see xref:apikit::apikit-asyncapi-module-reference.adoc[].
...
The subflow (`<sub-flow/>`) in `notifications.xml` implements the business logic for sending order confirmation messages by email and Slack. This file isn't generated by the scaffolder.  
...
. Posts the Slack confirmation message by using the *<slack:post-message/>*`* operation from Slack Connector 
. Emails the confirmation message by using the *<email:send/>* operation from Email Connector
...
* xref:apikit::apikit-asyncapi-module-reference.adoc#validation-of-messages[AsyncAPI JSON Schema Validation]
