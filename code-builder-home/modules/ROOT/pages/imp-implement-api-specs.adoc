...
Re-scaffolding enables you to update a project with the latest changes and version of an API spec in Anypoint Exchange. Re-scaffolding updates the configuration XML and `pom.xml` in your project. To re-scaffold a spec defined in a local API design project that is not published to Exchange, see xref:imp-implement-local-apis.adoc[], instead.
...
. Click the image:icon-tree-view.png["",15,15] (*Show Mule graphical mode*) icon in the activity bar to open the canvas if it doesn't open automatically.
