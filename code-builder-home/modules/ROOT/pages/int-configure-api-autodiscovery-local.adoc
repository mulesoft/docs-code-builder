...
Add API Autodiscovery from *Global Elements*:

. Open *Project Properties* > *Global Elements*.
. Click *New Configuration*, and select *API Autodiscovery*. 
. Complete the form and select the flow that Autodiscovery starts.

For more information, see xref:int-global-config-elements.adoc[].

Alternatively, add the autodiscovery element in your flow definitions. Use a property reference for `apiId` so you can load the value through configuration:
