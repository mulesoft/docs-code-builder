...
When Anypoint Code Builder can't process a value, it provides a visual indicator to identify an issue with the value. Issues include:
...
The DataWeave error `Unable to find sample input` applies to a Mule event variable, such as `payload`. In this case, Anypoint Code Builder places a wavy blue line under the value in the configuration XML file.

image::dw-issue-sample-data.png["'Unable to find sample input' error"]

If your application isn't running, Anypoint Code Builder can't provide any information about Mule variables unless you provide sample data.
