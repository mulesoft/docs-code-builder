...
Anypoint Code Builder includes bundled Mule runtime versions that enable instant project opening and design. When you run or debug applications, Anypoint Code Builder checks for the exact runtime version specified in your project. If the exact version isn't available locally, error dialogs appear to help you resolve the issue.
...
+
Anypoint Code Builder downloads and reinstalls the selected version. For details on selecting versions, see xref:int-versions.adoc#project-version-mule-java[Select Mule and Java Versions for a Project].
