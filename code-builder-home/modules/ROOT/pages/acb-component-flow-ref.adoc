...
To open the referenced flow, right-click the Flow Reference component and select *Go to Flow*. You can also press Ctrl+Alt+g (Windows) or Cmd+Option+g (macOS).
...
If the referenced flow is in an imported Mule project dependency, you can open it in a read-only canvas and view its read-only XML. You can inspect the flow and go to other flows in the dependency, but you can't modify the dependency flow or its XML.

This helps you trace and manage flow logic across modular designs, whether you're building or testing your application.

To find every flow that references a flow or subflow, see xref:int-configure-components.adoc#find-referencing-flows[Find Flows That Reference a Subflow].
