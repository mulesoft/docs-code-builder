...
* Verify that each project can run locally in your environment.
...
== Run Saved Configurations from Anypoint Code Builder

After MuleSoft Vibes creates a multi-project configuration, you can reuse it through standard execution paths.
...
|Runtime arguments
|Add `mule.runtime.args` to the launch configuration in your `.code-workspace` file, or update `Mule > Runtime: Default Arguments` in *Settings*.
