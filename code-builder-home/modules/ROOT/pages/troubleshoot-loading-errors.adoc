...
The error indicates that some Anypoint Code Builder dependencies have not finished loading. This issue typically occurs when you click *Design an API* and is often accompanied by the notification *Extracting Mule runtime: This operation may take a while...*. The extraction process can take several minutes, particularly after you install or update Anypoint Extension Pack.
...
To address this issue and avoid other errors, try logging in to Anypoint Platform, and wait for the project to finish loading before attempting to resolve any missing dependencies that remain.
