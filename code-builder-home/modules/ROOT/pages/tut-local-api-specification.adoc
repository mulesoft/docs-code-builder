...
. In the activity bar, click the image:icon-ms-logo.png["",20,20] (*Anypoint Code Builder*) icon to open Anypoint Code Builder.
...
----
. If prompted, sign in to Anypoint Platform, and follow all the prompts to reopen Anypoint Code Builder.
...
| *Project Name* | *Retrieve Users API* +
...
| *API Type* | *REST API* +
Anypoint Code Builder supports the REST API type for iterative design.
...
[[scaffold]]
== Scaffold the Local REST API Specification into an Interface
...
Local projects reside in Anypoint Code Builder.

To scaffold your local API specification:

. With `retrieve-users-api.raml` open, prepare to scaffold the specification into a Mule project:
+
...
. From your multi-root workspace in the Explorer, Anypoint Code Builder opens the scaffolded specification in your new Mule project.
+
...
----
* The canvas for this Mule project contains a Router component and several error handlers. APIkit automatically creates these components during the scaffolding process.
...
+
The status bar provides progress-related messages before Anypoint Code Builder updates your Mule project POM file to point to `v1.0.0` of your API specification:
...
. Import version 1.0.0 of your API from MuleSoft VCS:
...
. Implement the scaffolded specification.
