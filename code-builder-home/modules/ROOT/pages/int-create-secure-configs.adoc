...
To encrypt sensitive data, download the xref:mule-runtime::secure-configuration-properties.adoc#secure_props_tool[Secure Properties Tool Jar file].
...
.. Download the xref:mule-runtime::secure-configuration-properties.adoc#secure_props_tool[Secure Properties Tool Jar file].
...
Add, edit, or delete a secure properties configuration from *Global Elements*. To add one, click *New Configuration* and select the secure properties type. To delete one, click the *More actions* (*...*) menu next to the element and select *Delete*.

To edit the secure properties configuration:

. Open *Project Properties* > *Global Elements*.
+
image:int-global-elements-panel-list.png["Global Elements with a list of configurations including Secure Properties Config."]
+
. Select *global-configs.xml* from the *Selected File* dropdown.
. Find *Secure Properties Config* in the list.
. Click *More actions* (*...*) in the configuration name.
. Click *Edit*.
...
[[test-deploy]]
== Test Encrypted Properties in Anypoint Code Builder

If your application uses encrypted properties, add arguments for the encryption key and value to the runtime command. Adding these arguments to the setting enables the runtime to decrypt properties used in your application so that it deploys successfully to the local runtime.

The setting avoids an error that causes the deployment to fail when you use *Run and Debug*:
...
+
For example, for a file `dev.secure.yaml` that contains properties for the development environment:
+
[source,env-configuration]
----
-M-Denv=dev
----
. Deploy the application locally by pressing Cmd+shift+p (Mac) or Ctrl+shift+p (Windows), and entering *View: Show Run and Debug*, then clicking *Run and Debug*.
+
...
. If you intend to deploy your application to CloudHub or CloudHub 2.0, proceed to <<deploy-to-cloudhub>>.
+
...
When you deploy an application that you build with Anypoint Code Builder to CloudHub or CloudHub 2.0, hide the value of your encryption key to prevent Runtime Manager UI from displaying the value (see xref:cloudhub::cloudhub-manage-props.adoc[]). 
...
* Run and test encrypted properties used by your Mule application. See <<properties>> for guidance.
...
----
+
The _name_ (not the _value_) of your encryption key goes into a comma-separated `secureProperties` array. Later in this procedure, when you deploy your application to CloudHub, Anypoint Code Builder prompts you to provide the _value_ of the encryption key that you provided through the Secure Configuration Tool when encrypting your sensitive properties.
...
+
Other environments are not supported, and attempts to deploy to the *Design* environment fail with a `500` error code. While the deployment is in progress, Anypoint Code Builder provides the status of the process and prompts you to open the application in Runtime Manager when the deployment completes.
+
...
For more information, see xref:runtime-manager::index.adoc[]. For information about CloudHub, see xref:cloudhub::index.adoc[]. For information about CloudHub 2.0, see xref:cloudhub-2::index.adoc[]
...
When developing your application, create separate properties files for each of the target environments that require different settings, and provide the appropriate settings in each file.

[[filename-patterns]]
Typical filenames follow patterns like these:

* `dev.properties.yaml` or `dev.secure.yaml` for your development environment
...
//----
//+
//After deploying to CloudHub or CloudHub 2.0, you can add a new value through the Runtime Manager UI that takes precedence over this setting.
. _Before deploying to CloudHub or CloudHub 2.0_, ensure that the application deploys locally:
...
Open 'launch.json'
----
.. In `launch.json`, append the value of the `env` variable to use when running the application.
+
Notice the setting `-M-Denv=dev` in this `launch.json` example. For more information about launch configurations and alternative methods for passing properties, see xref:int-run-mule-apps-with-properties.adoc[].
+
[source,json]
----
{
...
.. If your application defines encrypted properties in your properties file, configure the *Runtime: Default Arguments* command with the encryption key and value so that the local deployment can decrypt the properties when the application runs.
+
...
| Encrypt a value
| `java -cp secure-properties-tool-j17.jar com.mulesoft.tools.SecurePropertiesTool string encrypt AES CBC <key> <value>`

| Decrypt a value
| `java -cp secure-properties-tool-j17.jar com.mulesoft.tools.SecurePropertiesTool string decrypt AES CBC <key> <encrypted-value>`
