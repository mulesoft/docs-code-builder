...
[[verify-mcp-tools-copilot]]
== Verify MuleSoft MCP Server Tools in GitHub Copilot

When you install Anypoint Code Builder, the MuleSoft MCP Server tools are automatically registered in VS Code. To confirm the tools are available in GitHub Copilot:

. Make sure GitHub Copilot is set up and you are logged in. For setup instructions, see xref:vibes-mcp-server.adoc#use-mcp-tools-copilot[Use MuleSoft MCP Server Tools with GitHub Copilot].
. Follow the VS Code instructions to https://code.visualstudio.com/docs/copilot/customization/mcp-servers#_use-mcp-tools-in-agent-mode[use MCP tools in agent mode^] and confirm that the MuleSoft tools appear in the tools list.
+
NOTE: The tools are registered directly in VS Code using the Language Model API. No manual configuration is required.
