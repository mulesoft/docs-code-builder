...
Manage user access to Anypoint Code Builder and Anypoint Platform features. Use Access Management to grant permissions directly to individual users or assign permissions to teams to which the users belong.
...
** *Viewer*
** *Creator*
* Set these xref:access-management::permissions-by-product.adoc#runtime-manager[Runtime Manager permissions] to enable services related to CloudHub and CloudHub 2.0, such as deploying applications:
** *Read applications*
** *Create applications*
** *Delete applications*
* Set the xref:access-management::permissions-by-product.adoc#design-center[Design Center Developer] permission only if users design API specifications in Design Center.
+
This permission isn't required to design API specifications locally in Anypoint Code Builder.

For a complete list of permissions, including *Anypoint Code Builder Developer* and *Mule Developer Generative AI User*, see xref:access-management::permissions-by-product.adoc[].
