...
[[find-referencing-flows]]
== Find Flows That Reference a Subflow

Find every flow that references a subflow without searching the project manually. Right-click the subflow in the canvas and select *Find References*. You can also press Shift+Alt+F12 (Windows and macOS).
...
You can also use *Find References* on a flow to locate every flow reference that calls it.

This helps you trace execution paths and assess the impact of changes across a project.

*Find References* lists only flow reference components that use a static flow or subflow name. It doesn't resolve dynamic names that use DataWeave expressions.
