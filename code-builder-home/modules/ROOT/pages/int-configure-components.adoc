...
If a component references another flow (for example, via a Flow Reference connector), you can quickly access the target flow in the canvas. Right-click the Flow Reference component and select *Go to Flow*. You can also press Ctrl+Alt+g (Windows) or Cmd+Option+g (macOS).

Anypoint Code Builder opens the target flow either in the application canvas or the MUnit canvas, depending on the context. If the target flow is in an imported Mule project dependency, you can open it in a read-only canvas and view its read-only XML. You can inspect the flow and go to other flows in the dependency, but you can't modify the dependency flow or its XML.
