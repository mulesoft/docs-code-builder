...
Anypoint Code Builder opens the target flow either in the application canvas or the MUnit canvas, depending on the context. If the target flow is in an imported Mule project dependency, it opens in a read-only canvas. You can inspect the flow and go to other flows in the dependency, but you can't modify the dependency flow or its XML.
