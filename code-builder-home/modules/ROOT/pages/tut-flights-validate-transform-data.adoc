...
+
If the transform component already has a variable defined, your new variable is the one with name `vars.NewVariable1`.
...
. After the *Create Flight Code Map* transform, click *Add Component* (+) and select *Core > Transform Message*.
+
If a second *Transform* already exists, select it and skip this step. 
...
. Select *Core Processors > Flow Control > Choice*.
...
. Select *Core Processors > Error Handling > Raise Error*.
