...
* xref:einstein-ai-connector::index.adoc[Anypoint Connector for Einstein AI (<ms-einstein-ai:/>)] (Einstein AI Connector)
...
* xref:mcp-connector::index.adoc[Anypoint Connector for MCP (MCP Connector) (<mcp:/>)]
...
To add, edit, or delete connector configurations for the whole project without selecting a component on the canvas, use *Global Elements*. See xref:int-global-config-elements.adoc[].
...
+
This configuration listens on `/accounts`, builds a JSON payload, maps it to Account fields inside `<salesforce:records/>`, and calls *Create*.
...
Exchange lookups in Anypoint Code Builder use platform search components, which provide a more consistent asset discovery and selection experience across supported Exchange flows.
...
+
image::int-add-component-connector-version.png["Exchange connectors with valid versions for the project"]
+
...
//Open config XML:
. {open-config-xml}, such as `my-project-name.xml`.
. Click image:icon-tree-view.png["",15,15] (*Show Mule graphical mode*) in the activity bar to open the canvas if it doesn't open automatically.
...
To change or upgrade connector versions later, see xref:int-versions.adoc#project-versions-connectors[Select Connector Versions for a Project].
