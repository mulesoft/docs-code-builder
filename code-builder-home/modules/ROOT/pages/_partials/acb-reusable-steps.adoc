...
[%collapsible]
====
* Select *Code* > *Settings* > *Configure Snippets*.
* Alternatively:
...
// tag::open-ide[]
// Open the ACB IDE
. In the activity bar, click the image:anypoint-code-builder::icon-ms-logo.png["",20,20] (*Anypoint Code Builder*) icon.
// end::open-ide[]
//
//
// tag::close-project[]
...
+
image:anypoint-code-builder::acb-activity-bar-select-cloud.png["Selecting the EU cloud from Anypoint Code Builder"]
...
+
When you publish your API spec to Exchange, Anypoint Code Builder requests the name of the business group.
...
. Wait for Anypoint Code Builder to load a list of matches to the name.
...
Make sure that you have access to AsyncAPI implementation features:
