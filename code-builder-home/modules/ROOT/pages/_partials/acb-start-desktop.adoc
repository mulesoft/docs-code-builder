...
include::partial$acb-hardware-storage.adoc[]

For more information about hardware and storage requirements, see xref:ref-hardware-storage-requirements.adoc[].
...
. Open VS Code.
. Find *Anypoint Extension Pack* from within VS Code or from https://marketplace.visualstudio.com/items?itemName=salesforce.mule-dx-extension-pack[Visual Studio Marketplace^].
+
[NOTE] 
...
. Confirm that the <<acb-menu, Anypoint Code Builder menu>> loads.

. If you have an Anypoint Platform account running in the EU cloud, change the *Mule: Control Plane* setting to *EU cloud*. 
+
For guidance, see xref:anypoint-code-builder::start-acb.adoc#change-clouds[Set Anypoint Code Builder to the EU or US Cloud].
. Follow the steps in xref:anypoint-code-builder::start-acb.adoc#login-ide[Log in to Anypoint Platform].
+
NOTE::
If an Anypoint Platform session is active in any of your browsers when you install Anypoint Extension Pack, the extension pack uses that session to sign in to Anypoint Platform. You can xref:anypoint-code-builder::start-acb.adoc#logout-status-bar[log out] or  xref:anypoint-code-builder::start-acb.adoc#logout-status-bar[sign in] from the status bar.
...
If Anypoint Extension Pack is installed, update to the latest version of the extension pack by following VS Code instructions in https://code.visualstudio.com/docs/configure/extensions/extension-marketplace#_update-an-extension-manually[Update an extension manually^].
