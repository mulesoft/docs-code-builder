...
Some Anypoint Code Builder features can be disabled or hidden based on your settings.

For any features that aren't working as expected, first xref:troubleshooting.adoc#check-logs[check for problems and issues].
...
Use the Activity Bar to enable or disable icons.
...
For more information about trust settings, see https://code.visualstudio.com/docs/editing/workspaces/workspace-trust[Trusting a workspace^] in the VS Code documentation.

[[modify-trust-settings]]
=== Modify Trust Settings

To modify your trust settings:
...
If a flow lacks a component or contains only a trigger, such as an HTTP Listener operation or the Scheduler, an error similar to the following appears in the *Problems* panel and from the code editor: 
