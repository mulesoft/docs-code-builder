...
+
image::add-choice-router-slack-escalation-canvas.png["A flow diagram with the Add Component button icon highlighted"]
. Type `choice` and select *Choice* under *Flow Control*:
. Add the `<when/>` and `<otherwise/>` elements:
+
image::sync-api-choice-when.png["Search result for 'when', with resulting highlighted text reading 'mule:when'"]
+
[source,XML]
--
...
+
image::add-email-operation-slack-integration-canvas.png["A Choice flow with the Escalated Add Component button icon highlighted"]
. Type `send` and select *Send* from the *Email* section:
+
image::send-email-slack-integration-canvas.png["A list of components in the search results section, with Send highlighted"]
...
+
image::salesforce-select-service.png["The Setup menu with Home and Service options highlighted"]
