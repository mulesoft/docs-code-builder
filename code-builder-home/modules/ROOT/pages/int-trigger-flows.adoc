...
This feature immediately invokes the Scheduler's own trigger; it doesn't bypass the Scheduler or create a temporary schedule. The flow runs exactly as it would on a natural tick, so breakpoints and interceptors behave the same way.
