...
. <<add-fragments-to-spec, Add the API fragment to your API spec>> using the `$ref` (OAS and AsyncAPI) or `!include` (RAML) directives.
...
* <<example-avro-api-frag,`example-asyncapi-avro-fragment`>> AsyncAPI spec Avro schema fragment
...
--
+
For OAS 3.0 APIs, scaffolding supports `$ref` API fragments from Exchange, including fragments with transitive dependencies. DataSense also resolves types defined in fragment files when you scaffold into a Mule project that uses a supported Mule runime version.
+
[tabs]
====
OAS Example::
