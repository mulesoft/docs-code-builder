...
* Currently, DataWeave cannot recognize the larger context to which scripts belong. Therefore, DataWeave requests sample data for all scripts within a single component. In the future, you will need to provide samples only once for each context.
* DataWeave displays an error when you don't add context variable samples to your script. This error is not accurate as as the script is valid regardless of the existence of samples. In the future, this error will be replaced by a warning, accurately reflecting the consequences of the lack of samples. Any preview attempts will fail.
