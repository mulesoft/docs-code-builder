...
//ACB UI links to ui-tut-integrate.adoc (DO NOT REMOVE WITHOUT UX/DEV ALIGNMENT)
...
.. Select *Mule runtime* and *Java* versions. 
+
...
+
image::int-flow-name-ui.png["Change name of flow through canvas."]
...
.. In the canvas, after your *Flow* component, click the image:icon-plus.png["",15,15] (*Add component*) icon:
+
...
----
In `<http:listener-config/>` and its child element `<http:listener-connection/>`, set the attributes to values provided in the example. 
+
Notice that the `name` attribute changes to `inbound-request`. The `doc:id` is randomly generated and does not require a change.
...
----
====
. From the canvas, add a Set Payload component after your HTTP Listener operation.
