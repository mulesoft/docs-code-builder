...
From the canvas UI::
+
Click *Flow name1* to open the configuration UI for the flow, change the flow name to `getFlights`, and click the check mark to set the new name.
+
...
. In the configuration XML, configure the attributes for the HTTP Listener operation `<http:listener-config/>`:
+
[source,xml]
----
...
. Configure the attributes for the HTTP Listener operation:
+
[tabs]
====
From the canvas UI::
+
Click the *Listener* operation to open the configuration UI.
+
In the configuration UI, set the HTTP Listener attributes to the following values:
+
image::tut-int-listener-component-ui.png["Configure the Listener operation through the configuration UI."]
+
[calloutlist]
...
----
====
. From the canvas UI, add a Set Payload component below your HTTP Listener operation.
...
From the canvas UI::
+
Click the Set Payload component to open the configuration UI.
+
In the configuration UI, set the attributes to the following values:
+
image::tut-int-set-payload-component-ui.png["Configure the Set Payload component through the configuration UI."]
+
[calloutlist]
