...
. Configure the attributes for the HTTP Listener operation `<http:listener/>`:
+
[tabs]
====
From the canvas UI::
+
Click the *Listener* component to open its configuration UI.
+
image::tut-int-listener-component-ui.png["Change name of flow through canvas UI."]
+
[calloutlist]
.. Change the component name to `HTTP /flights`.
.. Select `inbound-request` from the configuration reference dropdown menu.
.. Specify `flights` for the *Path* field.
