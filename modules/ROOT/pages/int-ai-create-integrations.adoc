...
* Ensure that you have the following permissions to use the Anypoint Code Builder Agentforce feature:
...
To use connectors to easily embed an agent or use an LLM from within your MuleSoft applications, click *Connect to an agent* or *Connect to an LLM* in the *Build Agentforce Integrations* card. For more information, see xref:connectors::agentforce::index.adoc[Agentforce Connector Overview] and xref:connectors::einstein-ai::index.adoc[Einstein AI Connector Overview].
To build your flow from scratch, click *Build a flow* in the *Start From Scratch* card.

[[craft-ai-prompt]]
== Craft Your Input

Before you create a flow, define what you want to achieve with the flow, for example, optimizing a business process. Craft your message using natural language, and keep in mind that the more concise and specific you make your message, the more effective Agentforce is when generating your flow. 
...
* Due to the sensitivity of LLMs and depending on which LLM is used, the same message might produce different responses. Sometimes, a similar message can produce very different responses. 
====
...
+
You see a list of your previous messages, up to the maximum of 50. 
...
. Inspect the generated code, and if you're satisfied, click the insert code icon to insert the code into your flow. 
+
...
After Agentforce builds the flow, you might need to fill in information specific to your configuration. 
