...
For example, if you want to create a flow that reads contacts from Salesforce in JSON format, you can enter a prompt describing that task. The prompt is then transformed into a series of the connector operations and processors needed to execute that task, in this case: Listener, Query, Transformer, and Logger. 

A flow is generated, which you can then further configure to suit your specific needs. 
...
+
Proofread your prompt to ensure that it is clear and comprehensive before you send it. Revise your prompt if necessary.
...
* Due to the sensitivity of LLMs and depending on which LLM is used, the same prompt might produce different responses; and sometimes, a similar prompt can produce very different responses. 
====
