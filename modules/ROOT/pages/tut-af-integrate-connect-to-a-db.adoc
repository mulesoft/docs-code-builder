...
--
+
NOTE: The `getFlights` flow contains an expected error: `The content of element 'flow' is not complete.` This error is present until you add a connector later in this procedure.
...
. From the canvas, click the image:icon-plus.png["",15,15] (*Add component*) icon after the HTTP Listener component.
...
.. Set *SQL Query Text* to `Select * FROM american`.
