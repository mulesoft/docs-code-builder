...
//TODO: IS THIS A WORKAROUND FOR NOW OR IS THIS JUST THE WAY IT'S GONNA BE FOREVER?
The IDE inserts all user snippets into the `<flow/>` or `<sub-flow/>` element that you are working on. If you create and insert a user snippet for a global configuration that must reside at the same level as the `<flow/>` or `<sub-flow/>`, and not within the element, manually move the XML to the correct location. For example, configurations for connections to a database must reside outside of the `<flow/>` element. To learn more about this configuration, see xref:connect-to-a-db.adoc[].
