...
Use built-in snippets or define your own to reuse code patterns in configuration files for your Mule applications. This practice avoids the need to retype the same pattern each time you use it, helps you work more quickly, and reduces the potential for configuration errors. Use snippets to provide configurations for any elements supported by the Mule DSL, such as commonly used connector operations, processing patterns performed by Core components, or larger XML configurations for multiple elements in the configuration XML.

Snippets are templates created in the JSON format. When you insert a snippet into the configuration XML for a Mule application, the snippet JSON transforms to XML automatically.  
...
+
After you save the new snippet, select the new snippet from the canvas to load the configuration XML for your application, the XML for the logger looks like this:
+
[source,xml]
----
...
----
+
For this example, the next step is to add the `level` and `message` value of your choice to the component. 

. Add any other snippets to your `mule-xml.json` by adding new fields. For example:
+
//TODO: THE DB SNIPPET IS MISCONFIGURED AND BREAKS STUFF. NEED WORKING CONFIG!
// 	ALSO THE NUMBERS IN THE JSON BREAK THE CONFIG IN THE IDE. YOU HAVE TO DELETE THEM. 
// 	CAN WE WRITE THE NUMBERS IN A WAY THAT DON'T BREAK THE CONFIG SO PEOPLE DON'T HAVE TO DELETE THEM MANUALLY?
[source,JSON]
--
{
    // Example:
