...
From the UI, Anypoint Code Builder inserts all user snippets into the `<flow/>` or `<sub-flow/>` element that you are working on. If you create and insert a user snippet for a global configuration that must reside at the same level as the `<flow/>` or `<sub-flow/>`, and not within either element, use <<autocomplete, auto-complete in the configuration XML>> to find and insert the snippet. Snippets are identified by their name and a two-dimensional box icon. For example, configurations for connections to a database must reside outside of the `<flow/>` element. To learn more about this configuration, see xref:connect-to-a-db.adoc[].
