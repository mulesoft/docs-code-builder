...
The IDE inserts all user snippets into the `<flow/>` or `<sub-flow/>` element that you are working on. If you create and insert a user snippet for a global configuration, such as a configuration for a database connection, which must be located outside of the `<flow/>` or `<sub-flow/>` element, manually move the configuration to the same level as the `<flow/>` or `<sub-flow/>`element. 
