...
The IDE inserts all user snippets into the `<flow/>` or `<sub-flow/>` element that you are working on. If you create and insert a user snippet for a global configuration that must reside at the same level as the `<flow/>` or `<sub-flow/>`, and not within either elements, manually move the XML to the correct location. For example, configurations for connections to a database must reside outside of the `<flow/>` element. To learn more about this configuration, see xref:connect-to-a-db.adoc[].
...
[[autocomplete]]
== Use Auto-Complete To Find and Insert Snippets into a Flow or Subflow
...
* To find and insert a built-in from the configuration XML, press (Ctrl+shift or Cmd+shift) to open the auto-complete menu, and select a built-in snippet. For example:
+
...
* To find and insert a user snippet from the configuration XML, find the name of the snippet (as configured in `mule-xml.json`), and insert the XML configuration for the snippet into an application flow. 
