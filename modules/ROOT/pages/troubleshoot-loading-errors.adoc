...
The error indicates that background Anypoint Code Builder processes for loading some dependencies are not complete. This issue typically occurs when you click *Design an API* and is often accompanied by the notification *Extracting Mule runtime: This operation may take a while...*. The extraction process can take several minutes, particularly after you install or update extensions to the desktop IDE.
