...
The error indicates that some Anypoint Code Builder dependencies have not loaded yet. This issue typically occurs when you click *Design an API* and is often accompanied by the notification *Extracting Mule runtime: This operation may take a while...*. The extraction process can take several minutes, particularly after you install or update Anypoint Extension Pack on the desktop IDE.
