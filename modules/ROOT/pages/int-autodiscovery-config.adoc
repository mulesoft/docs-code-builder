...
Autodiscovery configuration in a Mule app requires an API instance ID from a Mule Gateway API in API Manager that is associated with a supported API spec. To acquire and configure an API ID and Anypoint Platform credentials required for autodiscovery in a Mule app, see <<setup>>.
