...
created an integration or implementation project, or published the spec to Exchange:
...
If you have not already created an implementation project, use Anypoint Code Builder to scaffold the API in a new Mule project:
...
--
+
** Scaffolds your API into the new implementation project and opens the configuration XML file.
+
...
include::partial$acb-reusable-steps.adoc[tags="import-asset-exchange"]
+
...
--
** Updates the configuration XML file that contains the interface for your implementation project with the changes to the imported specification.
. Check for the changes to your implementation project.
+
For example, if the updated specification adds a new endpoint, the configuration XML for the implementation project has a new flow for the endpoint. 
. If the specification introduced a new version number, open the `pom.xml`, and search for the new version of the dependency for the `artifactID` of the re-scaffolded specification.
