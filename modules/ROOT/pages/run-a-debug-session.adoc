...
. In Anypoint Code Builder, select the *Run and Debug* icon, and click *Start Debugging (F5)*.
+
image::start-debugging.png["Run and Debug icon, and Start Debugging (F5) icon highlighted"]
...
. Select the *Step Over* button on your debug toolbar to move the execution to your next breakpoint.
. In the Debug sidebar on the *Variables* panel, expand the *Mule Message* and the *Payload* variables. You can inspect the database response as an array.
+
...
. Select the *Step Over* button until your execution moves to the `<set-payload>` component. Your payload is `application/java`.
. Select the *Step Over* button until your execution jumps to the `<logger>` component. Your payload is now transformed to `application/json`.
+
image::debuger-watch-expression-json.png[]
. Select the *Continue* button on your debug toolbar. The application returns the response from the database to your REST client.
+
[source,JSON]
--
[
