...
Set a home directory for downloaded assets, which include Mule runtime (`runtime/`) and Java (`java/`) versions for your implementations and integrations, as well as the `maven_settings.xml` file in `maven/` (see xref:int-test-munit.adoc[]).
