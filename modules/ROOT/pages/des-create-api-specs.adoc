...
Import an API spec from MuleSoft VCS and use Git or another source-control management system compatible with VS Code in the desktop or cloud IDE to keep your project in sync. For more information, see xref:des-designing-api-specs.adoc#anypoint-scm[Source Control for API Design Projects].
...
+
As you enter elements, use xref:start-discover-ui.adoc#auto-complete[auto-complete] (or press Ctrl+Space) to display available options within the context. 
. You can sync your changes with Anypoint SCM.
+
