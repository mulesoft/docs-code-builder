...
+
IMPORTANT: Ensure that you select the *API Fragment* tab.
...
+
--
If your API specification appears as a REST API rather than an API spec fragment, check that the
the `classifier` attribute in the `exchange.json` file is set to one of the following, depending on the language:
...
To add an API spec fragment to the project directory so that you can include them in your specification:
...
To verify that the fragment dependency was added, check the `dependencies` section of the `exchange.json` file. 
For example:
...
Depending on the API spec language:
...
+
Use xref:start-discover-ui.adoc#use-autocomplete[auto-complete] (Ctrl+Space) to display available options within each context.
For example:
+
...
OAS::
+
Use the `$ref` directive to add a fragment to the specification.
+
For example, this `$ref` directive adds the `AmericanFlightDataType` from `example-oas-fragment` as a data type:
+
[source,YAML]
--
schema:
...
RAML::
+
Use the `!include` directive to add a fragment to the specification.
+
...
. In the `fragments` folder, create a new file, such as `my-fragment` and add your example code.
+
.. Place the cursor where you want to add the fragment.
...
OAS::
+
Use the `$ref` directive to add the fragment from the `fragments` folder to your spec.
For example:
+
[source,YAML]
--
...
RAML::
+
Use the `!include` directive to add the fragment from the `examples` folder to your spec.
For example:
+
[source,RAML]
--
