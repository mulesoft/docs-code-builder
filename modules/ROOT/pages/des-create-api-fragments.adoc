...
+
You can also <<create-add-fragment-directly, create and add a fragment directly in your project>> without publishing it to Exchange.
...
+
--
If your API specification appears as a REST API rather than an API spec fragment, check that the `classifier` attribute in the `exchange.json` file is set to one of the following, depending on the language:
...
. Verify that the fragment dependency was added by checking the `dependencies` section of the `exchange.json` file. 
For example:
+
image::des-verify-frag-from-exchange.png["Verify the fragment dependency in the exchange.json file"]
+
You can also see the fragment in the Explorer view under *PROJECT DEPENDENCIES*.
For example:
+
image::des-api-frag-dependencies.png["API fragment in the Explorer view"]
+
The fragment files are located on your system in `~/.exchange_modules`.
