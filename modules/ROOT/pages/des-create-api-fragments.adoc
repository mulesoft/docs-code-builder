...
. <<add-fragment-to-spec, Add the API fragment to your API spec>> using the `$ref` (OAS and AsyncAPI) or `!include` (RAML) directives.
...
* <<example-avro-api-frag,`example-asyncapi-avro-fragment`>> AsyncAPI spec fragment
* <<example-asyncapi-spec,`example-asyncapi-spec`>> AsyncAPI spec
* <<example-asyncapi-spec-inline-avro,`example-asyncapi-spec-inline-avro`>> AsyncAPI spec (inline Avro)
...
+
As you enter elements, use xref:start-discover-ui.adoc#use-autocomplete[auto-complete] (or press Ctrl+Space) to display available options within the context.
+
...
. Use xref:start-discover-ui.adoc#use-autocomplete[auto-complete] (or press Ctrl+Space) to display available options within each context, for example:
+
...
[[example-avro-api-frag]]
== Example AsyncAPI Spec Fragment (Avro Schema)
...
If you created an AsyncAPI API spec project, you can replace the initial API spec with following example code to test adding the API spec fragments:
...
If you created an AsyncAPI API spec project, you can replace the initial API spec with following example code to test inlining the API spec fragments:
