...
The scaffolder generates a `flow.xml` file from the specification. This example generates the file from the <<ex-asyncapi-spec, AsyncAPI specification example>>:
...
--
[calloutlist]
. After the flow `LISTEN:listen-order-placed` is triggered by an order, an HTTP request operation (`<http:request/>`) requests and stores inventory data in the target variable `inventory`.
