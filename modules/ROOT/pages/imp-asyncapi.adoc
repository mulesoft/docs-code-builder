...
Anypoint Code Builder supports the implementation of AsyncAPI specifications. When creating an implementation project in Anypoint Code Builder, you can import and scaffold an AsyncAPI specification from Anypoint Exchange into an API interface that you can implement from the cloud or desktop IDE. The scaffolding process uses xref:apikit::apikit-for-asyncapi.adoc[APIkit for AsyncAPI] to create a set of flows for each `publish` operation in the specification and adds the xref:apikit::apikit-asyncapi-module-reference.adoc[APIkit for AsyncAPI Module] to the project. The module provides a Message Listener, a Publish operation, and set of error codes that you can handle with Error Handling components (see xref:apikit::apikit-asyncapi-error-handling-reference.adoc[]).
...
[[scaffold-new]]
== Scaffold an AsyncAPI Spec into a New Project

Create an implementation project from an AsyncAPI specification on Anypoint Exchange. When creating the project, Anypoint Code Builder scaffolds the spec into an interface that you can implement as a Mule app.

When scaffolding, Anypoint Code Builder:

. Introspects the API spec
. Creates a new Mule project with a flow for each `publish` operation in the spec, translating each operation to a listener and adding a Logger with message `payload` 
//TODO: NANDA
[NOTE]
If your AsyncAPI specification is not published to Exchange, see xref:design-center::TODO_TODO.xml[]. To create an AsyncAPI specification before publishing to Exchange, see xref:design-center::TODO_TODO.xml[].

For reference, the following procedure scaffolds the 2.6 AsyncAPI spec in <<examples>>:

. Create an implementation project from an AsyncAPI specification on Exchange:
.. Provide a name ...
.. Search for the AsyncAPI asset on Exchange. 
.. Add the asset.
.. Provide Mule runtime and Java versions for your specification.

. Review the scaffolded specification in your implementation project. 
.. TODO_TODO: 

[[scaffold-new]]
== Scaffold an AsyncAPI Spec into an Existing Project

You can import and scaffold an AsyncAPI spec from Exchange into an existing project in Anypoint Code Builder. This process is similar for other types of specs, such the xref:tut-af-implement-am-flights-api.adoc#import-and-scaffold-your-american-flights-api-specification[American Flights example].

[NOTE]
====
The AsyncAPI specification must be hosted on Exchange. For AsyncAPI specifications, iterative design and implementation is not supported at this time. For information about this process, see xref:imp-implement-local-apis.adoc[].
====

TODO_TODO: NAME OF SPEC, LINK TO INFO ON WHAT GETS SCAFFOLDED AND WHAT DOESN'T
...
== Implement an API in a Mule App

TODO_TODO

== Deploy an Implementation

Deploy the implementation to CloudHub or to another host. You can deploy to CloudHub from xref:int-deploy-mule-apps.adoc[Anypoint Code Builder] or Anypoint Runtime Manager. 

For deployment options, see xref:runtime-manager::deployment-strategies.adoc[]. 

[rescaffold]
== Re-scaffold an AsyncAPI Spec

If you rescaffold an updated AsyncAPI spec from Exchange into your implementation project, only new, as yet unscaffolded operations in a combined channel, operation, and server grouping are introduced into your project. In Anypoint Code Builder, existing flows in the project do not change. 

If a new `server` configuration is added to the spec, the scaffolder adds a new Subscribe 

If only a new server is added, a new configuration is added to the subscribe trigger
Any previously scaffolded operations that have been changed or modified will be ignored
If there is no channel/operation/server combination not previously scaffolded, we will show a notification and end the flow here

If one combination of channel, operation, and server is not yet scaffolded into your project, the flow is the same as for first time scaffolding, including UI, with the exception that we only consider newly added operations.

If the server is not supported, an error results. 
...
* If the spec defines at least one channel but no server is defined, the IDE prompts you for server details and defaults to the defined channel. 
* If the spec defines a server but the n is missing, TODO_TODO_WHAT_HAPPENS
* If a channel is defined without a `publish` or `subscribe` operation, TODO_TODO_IS_THERE_A_DEFAULT

If a spec defines only one server and a channel definition is missing a reference to a server, the scaffolder uses the defined server. If the spec defines multiple servers and a channel definition is missing a reference to a server, the IDE prompts you to identify the server to implement. 
...
The <<ex-asyncapi-spec, specification example>> defines server settings for an Anypoint MQ broker. To listen for and publish to an Anypoint MQ queue of operations in the Anypoint Connector for MQ (Anypoint MQ Connector) operations in your to listen or publish  scaffolding process for Anypoint MQ produces a properties file for a xref:mq::mq-client-apps.adoc[client app] you create to generate credentials to use for publishing and consuming from an Anypoint MQ queue. 
...
Then introspects channels in spec and configs operations from module in the project
...
Datasense: also sets up Datasense for incoming payload based on the message def in API spec -- so you can extract values and transform payload
...
Subscribe operations in spec translated to Publish ops in flow; user decides where to add the Publish op
