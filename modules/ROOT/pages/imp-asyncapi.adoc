...
* Review the scaffolding process for AsyncAPI specifications in <<scaffolding>>.
...
+
The scaffolder generates a `global-configs.xml` file that contains connection configurations for the *Message Listener* and *Publish* operations in the flow. The example describes the contents of the file and adds connection configurations for the Email and Slack connector operations. 
...
+
You can copy the URL from the region that you select when creating the queue. See the *copy* button in xref:mq-queues.adoc#create-a-queue[Create a Queue], and see xref:mq::mq-client-apps.adoc[].
...
. `order-backordered` configures Kafka channel for publishing (listening for) and subscribing to (publishing) backorders.
...
--
[calloutlist]
. `AMQ-prod` in `<apikit-asyncapi:server value="AMQ-prod" />` identifies an Anypoint MQ server connection. The value matches a `serverKey` configuration for the Message Listener. 
. `Kafka-prod` in `<apikit-asyncapi:server value="Kafka-prod" />` identifies a Kafka server connection. The value matches a `serverKey` configuration for the Message Listener. 

For more information, see <<ex-connection-configs-scaffolded>>.
--

[[ex-flows-implemented]]
The following example adds business logic to the scaffolded flows: 

* `LISTEN:listen-order-placed` listens for orders and, when triggered, uses a DataWeave expression to check for sufficient inventory. If enough items exist in the inventory, the flow publishes order data to the `order-confirmed` channel, which triggers the `LISTEN:listen-order-confirmations` flow. If not, the flow publishes order data to a `order-backordered` channel, which triggers the `LISTEN:listen-order-backordered` flow.
...
--
[calloutlist]
. After the flow `LISTEN:listen-order-placed` is triggered by an MQ client, an HTTP request operation (`<http:request/>`) returns sample order data and stores the data in the target variable `inventory`.
...
+
If the stock is available, the Set Payload component (`<set-payload/>`) configures a confirmation message for the *Publish* operation (<apikit-asyncapi:publish/>) to the `order-confirmed` channel. If not, the flow publishes a message to the `order-backordered` channel, which triggers the flow `LISTEN:listen-order-backordered`. 
...
*  *Publish* operations (`<apikit-asyncapi:publish/>`) in the flows rely on the connection configurations to publish to specified Kafka and Anypoint MQ channels. 
* *Message Listener* operations (`<apikit-asyncapi:message-listener/>`) in the flow are automatically configured to listen on a specific channel based on a server `value` setting (`"Kafka-prod"` or `"AMQ-prod"`). This value matches one of the `serverKey` settings in the AsyncAPI connection configuration (`<apikit-asyncapi:config/>`).
