...
+
Configure connector operations and processing components that define your business logic. Provide configuration property values to the `dev-properties.properties` file. Optionally, create additional flows for your processing requirements.  
+
For an example of an AsyncAPI specification, the scaffolded interface, and some business logic for processing orders within a Mule application, see <<examples>>. 
...
Opens a configuration XML file that contains the scaffolded flows (`flows.xml`) in the project.
. Provide business logic to implement this interface within the Mule app:
+
Configure connector operations and processing components that define your business logic. Provide configuration property values to the `dev-properties.properties` file. Optionally, create and configure additional flows or subflows for your processing requirements.  
+
For an example of an AsyncAPI specification, the scaffolded interface, and some business logic for processing orders within a Mule application, see <<examples>>. 
...
+
The subflow (`<sub-flow/>`) in `notifications.xml` implements the business logic for sending order confirmation messages by email and Slack. When triggered by the Flow Reference component in the `LISTEN:listen-order-confirmations` flow, the notifications subflow sends order confirmation messages defined in  `LISTEN:listen-order-confirmations`.
...
--
[calloutlist]
. After the flow `LISTEN:listen-order-placed` is triggered by the MQ client app, and HTTP request (`<http:request/>`) receives sample data for the order and stores the data in the target variable `inventory`.
. The Choice component (`<choice/>`) receives the sample data, and checks for available stock (`vars.inventory.stockDetails.availableStock`) from the target variable against a `quantity` value in the mock payload (`payload.items.quantity[0]`) that the flow received when it was triggered. 
+
If the stock is available, the Set Payload component (<set-payload/>) configures a confirmation message for the *Publish* operation (<apikit-asyncapi:publish/>) to the `order-confirmed` channel.
. The flow `LISTEN:listen-order-confirmations` listens for activity on the Anypoint MQ channel `order-cancelled`. When triggered, the flow configures its `<set-payload/>` component to create an order confirmation message for the Flow Reference (`<flow-ref/>`) to send for processing withing the `notification-flow` subflow.
...
When triggered by the Flow Reference component in the `LISTEN:listen-order-confirmations` flow, the notifications subflow sends order confirmation messages defined in `LISTEN:listen-order-confirmations`.
