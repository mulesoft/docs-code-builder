...
For protocols supported by these brokers, see xref:apikit::apikit-asyncapi-module-reference.adoc#supported-message-brokers[Supported Message Brokers] in APIkit for AsyncAPI Module Reference.
...
+
** **Publish (`<apikit-asyncapi:publish/>`): AsyncAPI `subscribe` operations in the specification are available for configuration as AsyncAPI *Publish* operations. See examples in <<ex-flows-scaffolded>>.
...
* For unsupported <<intro, message brokers>>
...
Scaffold or re-scaffold an AsyncAPI specification from Exchange into an existing project in Anypoint Code Builder. Before re-scaffolding, review <<scaffold>>.
...
* <<ex-flows-scaffolded>>
+
The scaffolder generates a `flow.xml` file with a flow for each `publish` operation in the AsyncAPI specification (for re-scaffolding behavior, see <<scaffold>>). The example describes the generated flows and configures components that provide order-processing logic to some of the flows. 
+
* <<ex-connection-configs-scaffolded>>
+
