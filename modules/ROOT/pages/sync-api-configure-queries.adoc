= Configure Your Queries to Your Services
...
. Replace it with a Transform Message copmonent using the *transform with output JSON* option:
+
...
With your query built, you can now use the Salesfroce Connector to retrieve the record from Salesforce, if the record exists.
...
. Move the execution forward until your notice that Anypoint Code Builder returns a `returnMessage` variable with the message `Failure: Contact does not exist in Saleforce`.
+
