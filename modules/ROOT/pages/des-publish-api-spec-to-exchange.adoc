= Publishing API Specifications to Anypoint Exchange

// tut-design-api-publish-api-spec-to-exchange.html

include::reuse::partial$beta-banner.adoc[tag="anypoint-code-builder"] 

//LOGO (web, desktop, or both)
include::partial$acb-ide-logos.adoc[tags="both-ides"]

Publish American Flights API specification to Exchange so that other team members can find and implement it.

== Before You Begin

Before publishing an API specification:

. xref:des-create-api-specs.adoc[Create an API specification].
// . xref:des-add-examples-to-specs.adoc[Add examples to your specification].
. xref:des-test-api-specs.adoc[Test your API specification using mocking service].
. xref:des-sync-with-design-center.adoc[Synchronize your API specification with Design Center].


== Publish an API to Exchange

To publish your API to Exchange:

. In Anypoint Code Builder, open the specification for your American Flights API project, such as `american-flights-api.raml`.
// Pointer to Command Palette
include::partial$acb-reusable-steps.adoc[use-command-palette]
+
[source,command]
----
MuleSoft: Publish API Specification to Exchange
----
+
//TODO: Cloud IDE only because it contains hamburger menu. Need to make generic. Plus, take a SS in a lower resolution, or only a part of the screen. Hi-res SS are hard to read in a 13 inch display.
//TODO duke: fix/redo image
image::publish-to-exchange.png["MuleSoft: Publish API Specification to Exchange highlighted in Command Palette"]
. Type a project name: +
* *American Flights API ACB*.
+
image::set-asset-name.png["American Flights API option highlighted"]
. Confirm the artifact ID: *American-Flights-API*
. Confirm the asset version: *1.0.0*.
. Confirm the API version: *v1*.
The status bar shows the progress.
+
// TODO: Crop the image to show the progress bar in its context. Cloud IDE only because it contains hamburger menu. Need to make generic. Plus, take a SS in a lower resolution, or only a part of the screen. Hi-res SS are hard to read in a 13 inch display.
// TODO duke: replace/fix image
image::publishing-to-exchange-dialog.png["Publishing API specification to Exchange notification highlighted"]

. When prompted to implement the API now, select *No* to avoid scaffolding the American Flights API specification into your integration.
For example:
+
image::scaffold-project-no.png["No option highlighted in the API implementation"]
+
Selecting *Yes* scaffolds the API specification into your project. Instead of scaffolding now, you will scaffold in xref:implement-api-specification.adoc[].

== Locate Your API in Exchange

After publishing your API specification, you can find it in Anypoint Exchange:

. Navigate to Anypoint Platform and login using your credentials.
. Navigate to Anypoint Exchange.
. Notice that your API specification is listed as an asset. +
You can select the API, navigate through its summary, and see all the endpoints you defined in the previous tasks.

== Next Step

Learn to xref:develop-integration.adoc[Develop an Integration].
