...
--
. Above `<http:listener-config/>`, click *Test Connection* to check the HTTP Listener connection. 
+
The UI provides status:

* *Verifying connection inbound-request*
* *Connection is valid* indicates a successful connection. Potential error for 
*Invalid Connection Got status code: 500 when trying to resolve a Mule Runtime operation* indicates a connection error.
+
One `500` error is `port 8081: Address already in use`. For port conflicts, configure a different port, such as `8082`, and retest the connection. 
. Proceed to <<run>>.
...
* For the desktop IDE, submit this UIR:
+
[source,URI]
----
http://0.0.0.0:8081/flights
----
* For the web IDE, submit a `GET` request to `+https://<web-IDE-instance>/proxy/8081/flights+`. 
+
See xref:ping-locally-deployed-app.adoc[] for guidance on constructing the URI.
