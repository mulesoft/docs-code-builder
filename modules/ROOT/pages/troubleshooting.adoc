...
When an issue that you cannot resolve occurs, the first step is to check for problems and issues. The *Problems* tab lists warnings and error messages. The *Output* panel provides build details and access to logs. Error messages and logs list underlying issues that can help you understand what went wrong and what requires attention. 

* To view project logs from the *Output* panel, see xref:start-open-panel.adoc[Output panel].
* To view problems, open the *Problems* tab by pressing Press Cmd+Shift+m (Mac) or Ctrl+Shift+m (Windows). 
