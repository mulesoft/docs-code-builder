...
:page-aliases: use-dataweave-to-transform-data.adoc
...
+
If the *User Snippets* pane is empty at first, wait a moment, and try again. Alternatively, navigate to the `muledx:transform-message` snippet from the configuration XML.
+
...
+
For guidance, see xref:tut-integrate-am-flights.adoc#run[Run Your Application in Debug Mode].
. After the application deploys successfully, use a REST Client or browser to trigger a response.
+
For guidance, see xref:tut-integrate-am-flights.adoc#test[Test Your Application].
...
+
For guidance, see xref:tut-integrate-am-flights.adoc#stop[Stop Your Application].
. Proceed to xref:tut-implement-am-flights-api.adoc[].
