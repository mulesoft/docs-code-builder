...
To review a condition later, hover the breakpoint in the gutter. A tooltip shows the configured expression in the form `Condition:` followed by your DataWeave expression.
...
+
Whenever execution pauses, the debugger re-evaluates each watch expression and shows the result next to the expression, similar to the *Debug Console*.
+
