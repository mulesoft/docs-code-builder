...
To add a breakpoint to a batch component in the canvas UI or in a configuration XML file:
...
NOTE: Batch execution is concurrent. Multiple record blocks can be processed in parallel, so breakpoints might trigger on different threads. The Debug panel displays each thread context separately so you can inspect each execution path.
