...
. Submit a request from the IDE's terminal, for example:
+
[source,URI]
----
...
----
+
See xref:int-trigger-flows.adoc#trigger-flow[Trigger a flow] for guidance with the URL.
...
. Make a `GET` request trigger your flow by using a `curl` command from the IDE's terminal, for example:

* From the desktop IDE, for example:
+
[source,URI]
----
> curl http://0.0.0.0:8081/flights
----
* From the cloud IDE, for example:
+
[source,URI]
----
> curl http://0.0.0.0:8081/api/flights
----
+
See xref:int-trigger-flows.adoc#trigger-flow[Trigger a flow] for guidance with the URL.
...
+
image::debugger-stop-db-select.png["Variables panel with details about the Mule event]
