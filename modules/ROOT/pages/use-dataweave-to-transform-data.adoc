...
. Type `_transform_`, and select the `Transform Message` snippet.
+
image::transform-with-output-json.png["Transform component highlighted"]
// TO-DO: Web IDE="Adding Components" and "Adding Content". Desktop IDE= "Add Component" and "Plus icon".
...
// TO-DO: It is not clear where the following expression should be placed. A "Check your code" would be helpful.
+
[source,dataweave]
--
%dw 2.0
...
+
image::transform-with-output-json.png["Transform component highlighted"]
// TO-DO: This image is repeated twice in this article. In the second case, we may look for another image.
...
The MySQL database provides the following response:
...
--
// To-Do: Get aligned with our CX style guide and change notes and warnings format from [WARNING]/[NOTE] to WARNING:/NOTE: (https://confluence.internal.salesforce.com/display/DOCTEAM/TCX+-+Notes%2C+Tips%2C+Warnings%2C+and+Important+Notes). 

Although you got a JSON response from your application, it's not the same JSON structure that your API specification dictates:

. Open your `American Flights API - Anypoint Code Builder` in Exchange.
. The following defined JSON example under your `GET` method for your `/flights` endpoint is provided:
+
...
--
+
image::dw-expression-transform-response.png["DataWeave example highlighted"]
...
See xref:ping-locally-deployed-app.adoc[] for more information about how to send request to your application.
+
...
--
. On Anypoint Code Builder, select the stop icon from the toolbar to stop your application.
