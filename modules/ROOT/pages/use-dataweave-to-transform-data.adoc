...
+
.Transform Message user snippet XML:
[source,xml]
----
...
To restructure the response data: 

. Use DataWeave to transform the response data:
...
+
For guidance, see xref:create-basic-integration.adoc#run[Run Your Application in Debug Mode].
. After the application deploy successfully, use a REST Client or browser to trigger the transformed response.
+
For guidance, see xref:create-basic-integration.adoc#test[Test Your Application].
+
