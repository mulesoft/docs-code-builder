...
. In your preferred REST client, create a `POST` request to the local address and send a `bookById` query:
// TODO: cc: I had to modify the query in order to get it to work. Is this different from the web? Or is it incorrect in web as well?
// query BookById {
//     bookById(id: "1") {
//         id
//         name
