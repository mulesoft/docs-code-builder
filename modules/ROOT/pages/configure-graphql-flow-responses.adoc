...
See xref:test-graphql-first-implementation.adoc[Test Your GraphQL Implementation]
...
. Using `curl`, create a `POST` request to the local address and send a `bookById` query. See xref:test-graphql-first-implementation.adoc[Test Your GraphQL Implementation] for examples.
