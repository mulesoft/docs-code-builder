...
Scaffold an API specification into an interface project within your IDE, and synchronize changes to the specification with the interface without first publishing to Anypoint Exchange. The interface is a configuration XML file automatically created during the scaffolding process. 
...
+
Your API specification file `retrieve-users-api.raml` opens automatically from the Explorer menu. You can also open in manually.
...
+
The UI provides a message *Implementing local API implementation, please wait.* Wait for up to a minute for the field *Mule project name* to appear at the top of the UI. For example:
+
