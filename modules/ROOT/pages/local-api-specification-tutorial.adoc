...
Anypoint Code Builder enables you to scaffold an API specification into the configuration XML for Mule project without first publishing the specification to Exchange. For example, if you add a new endpoint to your API specification, Anypoint Code Builder is able to scaffold a new flow for that endpoint within your IDE.
...
+
The IDE permits only one iterative design and implementation project per VS Code workspace. If you create an API specification from a multi-root workspace, the specification automatically belongs to that workspace. An API specification created from a new VS Code window does not belong to any multi-root workspace.
+
Creating a new window for this procedure enables you to create a multi-root workspace in an upcoming step and avoids an issue that occurs if you create a specification in a multi-root workspace that is already using iterative design with another API specification. 
