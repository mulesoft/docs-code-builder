...
Scaffold an API specification into an interface within your IDE, and synchronize changes to the specification with interface without first publishing to Anypoint Exchange. The interface is a configuration XML file automatically created during the scaffolding process.

For example, after adding a new endpoint to your API specification, you can re-scaffold the change into your implementation XML to add a flow for that endpoint.
...
To avoid potential issues from limitations of multi-root workspaces when implementing a local API specification, start with a new VS Code window and folder for your workspace. 
...
.. In *Project Location*, select *Browse*, create a folder for your project's multi-root workspace, and select that folder's location. For example:
+
`/Users/me/workspace-retrieve-users`
+
NOTE::
...
+
This process creates a project that contains your API specification in within your workspace.
+
...
.. Start the scaffolding process by entering a name for your Mule implementation project, such as `retrieve-users-implementation`, and selecting your workspace folder as the target folder the project.
+
...
* In the implementation XML, the flow `<flow name="get:\users:design-an-api-config"/>` contains an `<ee:transform/>` element with your example:
+
[source,XML,linenums]
--
...
After committing your changes to Design Center, publish the API specification to Anypoint Exchange to make discoverable by other team members.
...
The specification is now available on https://anypoint.mulesoft.com/exchange/?show=org&sort=last[Anypoint Exchange^] for team members in your business group to scaffold into their projects and implement. 

Use the command `Implement an API Specification` to get started with this process:
