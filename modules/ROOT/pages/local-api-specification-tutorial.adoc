...
To avoid potential issues from limitations of multi-root workspaces when implementing a local API specification, start with a new VS Code window: 
...
+
In the file menu that opens, create a folder such as `workspace-retrieve-users` for your project's multi-root workspace, and then select that folder's location.
+
...
. Click  *Select target folder*. 
+
This is the workspace folder you created when starting your API specification.
+
