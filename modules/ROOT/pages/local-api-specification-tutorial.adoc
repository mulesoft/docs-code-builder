...
Anypoint Code Builder enables you to scaffold an API specification into the configuration XML for a Mule project without first publishing the specification to Anypoint Exchange. For example, if you add a new endpoint to your API specification, Anypoint Code Builder is able to scaffold a new flow for that endpoint through the command `MuleSoft: Implement this local API`. Generate the Mule project with this command instead of attempting to use an existing project. 

A local project for an API specification or implementation resides in your Anypoint Code Builder IDE, whether in your desktop IDE or in the web IDE associated with your user account in Anypoint Platform. 
...
Iterative API design and implementation within the IDE takes place in a multi-root workspace in VS Code. The IDE permits only one pair of iterative design and implementation projects per multi-root workspace. 

To avoid potential issues from limitations of multi-root workspaces when implementing a local API specification, start with a new VS Code window and parent folder for your API specification project:
...
Synchronizing your local API specification with a local implementation project in your IDE requires a multi-root workspace in Visual Studio Code.

TIP: You can close a multi-root workspace with the command `Workspaces: Close Workspace`. Reopen a synchronized project from the workspace that you create when implementing the API.
...
+
If you don't find `MuleSoft: Implement this Local API` in the command palette or the X-shaped icon for this command , go to <<display-command>>.
+
images::local-api-spec-implementation.png["UI with the X-shaped icon for the command Implement this Local API"]
...
. Proceed to <<create>> before starting <<implement-local>> again.
