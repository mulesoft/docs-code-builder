...
+
_Do not select_ the command `MuleSoft: Implement an API Specification`, which is for implementing a specification that is published on Anypoint Exchange and _not for_ implementing a local API specification that is not published to Exchange.  
//+
//TODO duke: STILL WORKING ON GETTING FIX AT THE ANCHOR LINK TO WORK:
...
. After waiting for up to a minute for the field *Mule project name* to appear in the UI, start the scaffolding process by entering a name for your Mule project in that field. 
+
...
//TODO duke: TBD IF THIS WORKS OR IS NECESSARY:
//[TIP]
//--
...
//--
. Open the configuration XML for the new Mule project with the scaffolded specification from your multi-root workspace. 
+
...
. In Anypoint Code Builder, select the *SOURCE CONTROL* view of the IDE by pressing Caret(^)+Shift+g (Mac).
