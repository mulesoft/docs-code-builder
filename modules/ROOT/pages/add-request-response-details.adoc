...
. The status bar displays a *Dependencies successfully added* message.
. Open the Command Palette by pressing Ctrl+Shift+p (Windows) or Cmd+Shift+p (Mac), and provide the following command:
+
[source,command]
----
...
. Add an `!include` directive, and press Ctrl+Spacebar to open the autocomplete options.
...
. Press Ctrl+Spacebar to open the autocomplete options, and add the following elements:
* `responses`
* `200`
* `body`
* `application/json`
...
. Add a new resource at the same level as your `type` response by pressing Ctrl+Spacebar (Windows) or Cmd+Spacebar (Mac) to open the autocomplete options, and select `examples`.
// TODO hn: This automatically adds the colon and indents the next line so indention not necessary
. Under examples, add an indentation, press Ctrl+Spacebar, and select *output*.
// TODO hn: It doesn't add the colon - is it necessary?
. Add an `!include` directive and press Ctrl+Spacebar to open the autocomplete options.
// TODO You have to select examples or exchange_modules before you can select the raml...
. Select `AmericanFlightsExample.raml` as the reference for your example.
// TODO hn: my options don't include the .raml. When I include, I get an error that it doesn't exist. It works if I add "examples" in the path.
// TODO hn: I think there's a bug.
+
[source,RAML]
--
/flights:
...
. Press Ctrl+Spacebar (Mac) to open the autocomplete options and generate a 201 response:
+
[source,raml,linenums]
--
#%RAML 1.0
