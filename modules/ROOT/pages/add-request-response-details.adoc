...
/flights:
  ...
  /{ID}:
    get:
--
. Add an `!include` directive and press `ctrl + spacebar` to bring up the auto-complete options.
...
/flights:
  ...
  /{ID}:
    get:
--
. Add a new child element inside `queryParameters`, press `ctrl + spacebar` to bring up the auto-complete options, and add the elements:
* `responses`
* `200`
* `body`
* `application/json`
...
. Press `ctrl + spacebar` to bring up the auto-complete options to create a 201 response:
+
[source,raml,linenums]
--
#%RAML 1.0
