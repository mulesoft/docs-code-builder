...
/flights:
  ...
  /{ID}:
    get:
--
. Add an `!include` directive and press Ctrl+Spacebar open the autocomplete options.
...
/flights:
  ...
  /{ID}:
    get:
--
. Add a new child element at the same level as `queryParameters`, right after:
+
...
--
+
. Press Ctrl+Spacebar to open the autocomplete options, and add the elements:
* `responses`
* `200`
* `body`
* `application/json`
...
. Add a new resource at the same level as your `type` response by pressing Ctrl+Spacebar (Windows) or Cmd+Spacebar (Mac) to open the autocomplete options and selecting `examples`.
...
. Press Ctrl+Spacebar (Mac) to open the autocomplete options to create a 201 response:
+
[source,raml,linenums]
--
#%RAML 1.0
