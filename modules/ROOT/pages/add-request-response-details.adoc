...
Continue work in xref:design-api-specification-from-scratch.adoc[] by adding API fragments from Exchange for a data type and example. Then create your own examples for the specification, and add a custom message for a response. 

== Before You Begin

If you haven't started the American Flight API example, follow these procedures before attempting to add examples:
...
. Add the example to as a response to the `post` method, and include a custom message:
+
[source,RAML]
----
#%RAML 1.0
