...
* To generate a DataWeave script using *Map and Transform with AI*, provide input and output samples. While optional, providing metadata helps the AI better understand the data structure and improves the accuracy of the transformation.
...
* For automatic field-to-field mapping using *Map with AI*, you must provide both input and output metadata. This auto-mapping only works if there is no existing DataWeave script in the component.
...
The *Map with AI* feature automatically suggests a field-to-field mapping. This feature requires you to provide both input and output metadata for the component and only triggers if no DataWeave script is present. It performs auto-mapping based on the provided metadata.
