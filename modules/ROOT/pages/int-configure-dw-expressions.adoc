...
== Transform Mapping Use Cases

This table details the supported transform mapping use cases.

.Transform Mapping Use Case
[%header,cols="1a,1a"]
|===
| Transform Mapping Use Case | Supported
| `Object` to `Object` | Yes
| `Primitive` to `Primitive` | Yes
| `Object` to `Primitive` | Yes
| `Primitive` to `Object` | Yes
| `Object` to `Array` | Yes
| `Array` to `Object` | Yes
| Type Conversion `Object` to `Object` | Yes
| Concatenation of simple fields | Yes
| `Array` to `Array` (same level) | Yes
| `Array` to `Array` (different levels) | No
| Nested `array` mappings | No
| Concatenation within `arrays` of different levels | No
| Type conversion `Array` to `array` at different levels | No
|===
...
+
The mapped fields display a green dot and are highlighted to indicate that the mapping, the *Script* section updates with the new mapping, and the *Output Preview*"* section displays the output of the transformation.
+
If needed, map more than one source field to a target field and the Transform Builder concatenates them.
. Click on the *More Actions* icon to refresh metadata or remove all mappings.
