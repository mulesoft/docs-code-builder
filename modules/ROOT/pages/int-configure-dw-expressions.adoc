...
// TODO: Review this screenshot. It's too large for the page.
...
* Sample data provides the context to generate functions or expressions for calculations, conditions, etc (WSDL files can be provided with sample data).
* See <<sample-data>> for supported data formats (MIME types) for sample data.
* A loading indicator is shown when Einstein generates the transformation.
* You can’t change sample data when Einstein generates the transformation.
* History isn't available in *Transformation Builder*.
* Enter valid data to create the script transformation with AI, or write it manually.
* Chat history is not available in *Expression Builder*.

[[map-with-ai]]
=== Map with AI

Get a field-to-field mapping based on the sample data provided. When you access *Transformation Builder*, the mapping starts automatically considering the sample data provided.

//TODO: complete the prereq to make the "Map with AI" button not grayed out.
. Click on *Map with AI*.
. 

[[map-and-transform-with-ai]]
=== Map and Transform with AI

Create a DataWeave transformation script based on the sample data provided. 
...
. Review the generated scripts and insert one if appropriate, or copy a suggested script to the clipboard to make changes.
+
