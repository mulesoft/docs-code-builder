...
* To generate a DataWeave script, provide input and output samples. While optional, providing metadata helps the AI better understand the data structure and improves the accuracy of the transformation.
...
* For automatic field-to-field mapping without samples, you must provide both input and output payloads. This auto-mapping only works if there is no existing DataWeave script in the component.
...
With *Map with AI*, you can automatically generate a DataWeave transformation. There are two ways to use this feature:

* *Using Samples*: Provide input and output samples to generate a complete DataWeave script. This is the recommended approach.
* *Using Payloads*: If you provide both input and output payloads for a component with no existing DataWeave script, the feature suggests a basic field-to-field mapping.
...
* Review that the provided input and output metadata are valid if you choose to use them.
