...
DataWeave is the programming language designed by MuleSoft for data transformation and expressions in connectors and components. When you create an implementation or integration in Anypoint Code Builder, you are creating a Mule app that runs on an instance of the Mule runtime engine. 
...
Most connector operations and components have at least one expression field. In the configuration panel for a component, these fields are identified by an *fx* above the field. In the XML, these fields have `\#[]` markup. Expression fields accept DataWeave expressions, including DataWeave functions and Mule event data, such as `payload`, `attributes`, and `vars`. 

Open a component's expression field: 
...
+
image::int-dw-fx-field-set-variable.png["Expression builder for Set Variable"]
+
...
+
image::int-dw-fx-field-core-autocomplete.png["Expression builder for Set Variable"]
+
