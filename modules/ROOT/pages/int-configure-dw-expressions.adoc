...
* For automatic mapping of fields, provide both input and output payloads. Auto-mapping works if there is no existing DataWeave script in the component.
...
The *Map with AI* feature suggests an automatic field-to-field mapping when you open the *Transformation Builder*. This auto-mapping feature requires you to provide both input and output payloads for the component, and it triggers the auto-mapping feature if no DataWeave script is present. *Map with AI* also generates mappings using input and output samples, without requiring full payloads.
