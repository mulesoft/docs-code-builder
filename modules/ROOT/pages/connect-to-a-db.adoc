= Connect to a Database through the American Flights Example
:page-aliases: tut-dev-connect-to-a-db.adoc

include::reuse::partial$beta-banner.adoc[tag="anypoint-code-builder"]

//LOGO (web, desktop, or both)
include::partial$acb-ide-logos.adoc[tags="both-ides"]

Use Anypoint Connector for Database (Database Connector) to connect to an external database from your Mule flow.

== Before You Begin

. Complete American Flights API specification. Start from xref:design-api-specification-from-scratch.adoc[], and complete the remaining tasks for adding examples, testing with the mocking service, synchronizing with Design Center, and publishing to Exchange.
. xref:create-xml-snippets.adoc[]
. xref:create-basic-integration.adoc[] 

== Open Your American Flights Application

. In Anypoint Code Builder, open `american-ws-anypoint-code-builder.xml` file.
. Remove the `<set-payload>` component from the configuration XML:
+
[source,xml]
--
<set-payload value="Flight info" doc:name="Set Response" />
--

== Add a Database Driver Dependency

The Database Connector supports a variety of Java Database Connectivity (JDBC) databases. 

Provide the dependency for the `mySQL` driver: 

. In Explorer, select the `pom.xml` file.
. Add a new dependency:
+
[source,xml]
--
<dependency>
   <groupId>mysql</groupId>
   <artifactId>mysql-connector-java</artifactId>
   <version>8.0.29</version>
</dependency>
--
+
image::add-mysql-driver-dependency.png["Pom.xml and dependency highlighted"]
//TODO: hamburger menu
. Replace the existing `<configuration/>` element within your `mule-maven-plugin` with a new shared library:
+
[source,xml,linenums]
--
<plugin>
    <groupId>org.mule.tools.maven</groupId>
    <artifactId>mule-maven-plugin</artifactId>
    <version>${mule.maven.plugin.version}</version>
    <extensions>true</extensions>
    <configuration> //<1>
     <sharedLibraries>
       <sharedLibrary>
         <groupId>mysql</groupId>
         <artifactId>mysql-connector-java</artifactId>
       </sharedLibrary>
     </sharedLibraries>
    </configuration>
</plugin>
--
[calloutlist]
.. Add a shared library configuration.
. Proceed to <<add-db-config>>. 

[[add-db-config]]
== Add a Database Connector Configuration

. In Anypoint Code Builder, open `american-ws-anypoint-code-builder.xml`.
. In the configuration XML, add the global `<db:config/>` snippet above the `<flow>` element:
.. Press Ctrl+Spacebar (Mac) or Cmd+Spacebar (Windows), type `mysql`, and select the snippet `db:mysql-config` to produce the following XML configuration above the `<flow/>` component:
+
.Snippet above the <flow/> element:
[source,xml]
----
<db:config name="Mysql_Database_Config" >
  <db:my-sql-connection host="${mysql.host}" port="${mysql.port}" user="${mysql.username}" password="${mysql.password}" database="${mysql.database}" />
</db:config>

<flow name="getFlights" >
...
---- 
.. Edit the attributes in `<db:config/>` to produce this XML configuration:
+
[source,xml]
--
<db:config name="Database_Config" doc:name="mySQL DB">
    <db:my-sql-connection
      host="mudb.learn.mulesoft.com"
      port="3306"
      user="mule"
      password="mule"
      database="training" />
</db:config>
--
. Review the complete configuration XML:
+
[source,XML]
--
<http:listener-config name="inbound-request" doc:name="HTTP Config">
    <http:listener-connection host="0.0.0.0" port="8081" />
</http:listener-config>

<db:config name="Database_Config" doc:name="mySQL DB">
    <db:my-sql-connection host="mudb.learn.mulesoft.com" port="3306" 
        user="mule" password="mule" database="training" />
</db:config>

<flow name="getFlights">
    <http:listener path="flights" config-ref="inbound-request" 
          doc:name="HTTP /flights" />
</flow>
--
. Proceed to <<write>>.

[[write]]
== Write a Query to Return All Flights

. From the UI canvas, select the `plus` icon below the HTTP Listener component, and add the user snippet *Database Select*.
. Provide this configuration for the snippet:
+
[source,XML]
--
<db:select doc:name="Query Flights" config-ref="Database_Config" >
  <db:sql>
    <![CDATA[Select * FROM american]]>
  </db:sql>
</db:select>
--
. Review the complete configuration XML:
+
[source,XML]
--
<http:listener-config name="inbound-request" doc:name="HTTP Config">
    <http:listener-connection host="0.0.0.0" port="8081" />
</http:listener-config>

<db:config name="Database_Config" doc:name="mySQL DB">
    <db:my-sql-connection host="mudb.learn.mulesoft.com" port="3306" 
        user="mule" password="mule" database="training" />
</db:config>

<flow name="getFlights">
    <http:listener path="flights" config-ref="inbound-request" doc:name="HTTP /flights" />
    <db:select doc:name="Query Flights" config-ref="Database_Config" doc:id="qcnfxf" >
        <db:sql>
            <![CDATA[Select * FROM american]]>
        </db:sql>
    </db:select>
</flow>
--
. Proceed to xref:use-dataweave-to-transform-data.adoc[]. 
+
Use DataWeave to make the database response match examples in your American Flight API specification. 