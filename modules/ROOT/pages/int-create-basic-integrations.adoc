...
. xref:int-create-xml-snippets.adoc[]
. xref:int-connect-databases.adoc[]
. xref:use-dw-to-transform-data.adoc[]
...
| *Project Location* | Your home directory or another directory you create (see xref:work-on-home-directory.adoc[]). Click *Browse* to select a different directory for the integration.
