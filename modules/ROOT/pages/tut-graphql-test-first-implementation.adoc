...
. After the application builds and deploys successfully in the IDE, click the image:icon-run-debug.png["",15,15] (*Run and Debug*) icon in the activity bar, then click the *Start Debugging (F5)* icon.
//open terminal
include::anypoint-code-builder::partial$acb-reusable-steps.adoc[tag="open-terminal"]
. In the terminal, open a new command prompt by clicking the plus (*+*) icon.
+
Alternatively, click the up arrow (*^*), and select a shell to use, such as *bash*. 
...
--
+
Mock data in the output comes from strings in the Set Payload component that is part of the  *Query.bookById* flow (`<flow name="Query.bookById"/>`):
+
[source, xml]
----
<set-payload value="{&#xa;  &quot;id&quot;: &quot;some-id&quot;,&#xa;
