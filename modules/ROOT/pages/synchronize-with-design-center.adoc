...
After following the procedures in xref:test-api-specification.adoc[], use the built-in integrated source control management (SCM) to sync American Flights API with Anypoint Design Center. (In addition to source management through the SCM, Anypoint Code Builder also supports Git.)

== Before You Begin

If you haven't started the American Flight API example, follow these procedures before attempting to synchronize:
