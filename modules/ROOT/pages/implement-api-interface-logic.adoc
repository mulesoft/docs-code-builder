...
. In Anypoint Code Builder Explorer view, right-click your `american-flight-api-acb.xml` file and select *Rename*.
+
image::api-interface-rename.png["Rename option highlighted the context menu of american-flight-api-acb.xml file"]
// TO-DO: The background of the contextual menu is white in the web IDE and grey in the desktop IDE.
. Rename the file to *interface.xml*.
+
image::interface-rename.png["interface.mxl file highlighted when renaming a file"]
. Right-click your `american-ws-anypoint-code-builder.xml` and select *Rename*.
+
image::api-implementation-rename.png["Rename option highlighted the context menu of american-ws-anypoint-code-builder.xml file"]
. Rename the file to *implementation.xml*.
+
image::implementation-rename.png["implementation.xml file highlighted when renaming a file"]
...
. In the graphical canvas, select the *HTTP /flights* listener. The corresponding XML code is shown.
+
