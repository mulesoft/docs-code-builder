...
[calloutlist]
.. Uses `!include` for RAML files or `$ref` for OAS 3.0 files to add the `AmericanFlight` object from `AmericanFlightDataType.raml` as a data type
+
Note that you can use built-in functionality to insert the `!include` or `$ref` directives and file path from the specification file. For example:
+
[loweralpha]
... After typing `AmericanFlight:&nbsp;`(with a space at the end), click Ctrl+Spacebar, start typing `!include` or `$ref`, and select it from the drop-down menu in the RAML file. For example:
+
image::tut-api-include-directive.png["Include directive in RAML drop-down menu"]
... Follow the same process to add the example, selecting `exchange_modules` directory and progressing step-by-step to `AmericanFlightsDataType`. For example:
+
...
.. Adds the `AmericanFlightsExample` example
...
. Right-click your `examples` folder, and create a new file named `AmericanFlightExample`.
+
...
--
====  
. Return to your `american-flights-api` file, and create a set of elements under `/{ID}/get` for responses:
+
[tabs]
====
          ...
          body: //<2>
            application/json:
              schema:
                type: AmericanFlight[]
			examples: //<3>
		output: $ref:'#/components/examples/AmericanFlightExample'
--
...
. Under your `/examples` folder, create a file named `AmericanFlightNoIDExample`, and copy this example to the file:
+
[tabs]
====
OAS Example::
...
. In the `american-flights-api` file, add the example as a response to the `post` method, and include a custom message.
+
...
type: AmericanFlight[]
			examples: //<3>
		output: $ref:'#/components/examples/AmericanFlighNOIDExample'
		responses:
		'201':
          content:
            application/json: //<4>
...
.. Post an `AmericanFlight` object from an `examples/AmericanFlightNoIDExample`
.. Provide a custom message for a response in JSON format to the `post` method
.. Add your example from `examples/AmericanFlightExample` as a response to the nested `get` request
//next step:
