...
Encrypt sensitive values of properties used in your Mule application, and hide the name of the encryption key in Runtime Manager when the application deploys to CloudHub. 
...
Encrypt one or more properties using the MuleSoft Secure Properties Tool. After encrypting, create a file for the properties. In the file, specify key-value pairs for which each encrypted property is the value and the key is a unique name of your choice, for example, `my.property : "![En8lII21ZHrdIaINw0+mSA==]"`. Specify the key-value pairs in a `.yaml` or Spring-formatted `.properties` file, and put the file in the `src/main/resources` directory of the project for your Mule application.
