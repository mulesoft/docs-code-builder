...
* `${_property-key_}` for unencrypted
+
For example, you use `${host}` in the XML for the property `host: "8081"`.
* `${secure::_property-key_}` for encrypted values
+
For example, you use `${secure::pw}` in the XML for the property `pw: "![n2LDIOKKxhQ5Tdv5AeCtFQ==]"`.
