...
The file names must follow a consistent pattern so that you can configure a variable such as `$\{env}` as a modifiable prefix that enables you to switch to the properties file with settings for the environment. You can include encrypted and unencrypted property values in the files.
