...
* If you are using the web IDE, ensure that you have the required permission: *Anypoint Code Builder Developer*. See xref:configure-permissions.adoc[].
...
. Launch the `deploy.json` file for your application by pressing Cmd+shift+p (Windows) or Ctrl+shift+p (Mac) and providing this command:
+
[source,command]
----
MuleSoft: Deploy to CloudHub
