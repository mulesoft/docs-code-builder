...
To provide property settings for different environments, such as production and development environments, create separate properties files for your environments, and use a variable in the file name in the `file` attribute of `<secure-properties:config/>` instead of hardcoding the entire file name. For instructions, see <<create-prop-file>>.
...
. Provide a local key-value pair for your encryption key name and its value through the UI or in `launch.json`:
...
* In `launch.json`:
.. Press Cmd+shift+p (Windows) or Ctrl+shift+p (Mac), and provide this command:
+
[source,command]
----
Open `launch.json`
----
.. Append the argument for the key-value pair as a value of the `"mule.runtime.args"` key. For example:
+
[source,launch.json]
----
"mule.runtime.args": "${config:mule.runtime.defaultArguments} -M-Dencryption.key=my-key-value"
----
+
In this example, `-M-Dencryption.key=my-key-value` sets the default name of the encryption key (`encryption.key`) and a value of `my-key-value`. 
+
Your encryption key value must match the value that you use to <<encrypt, encrypt your properties with the MuleSoft Secure Properties Tool>>, and your key name must match <<use, the name that you provide for it in `<secure-properties:config/>`>>. 
+
    ...
    "configurations": [
      {
        ...
        "mule.runtime.args": "${config:mule.runtime.defaultArguments} -M-Dencryption.key=my-key-value -M-Denv=dev"
      }
    ]
}
----
+
If you are also using a variable to define the name of your properties file in `<secure-properties:config/>`, you can append an argument with an `-M-D` prefix to define that variable, such as `-M-Denv=dev`. See <<create-prop-file>> for more information about this variable.
+
The complete `launch.json` example adds arguments `-M-Denv=dev -M-Dencryption.key=my-key-value` to define the `env` variable used in the properties file and the encryption key and value. 
...
.. In the IDE, press Cmd+shift+p (Windows) or Ctrl+shift+p (Mac), and provide this command:
+
[source,command]
----
Open `launch.json`
...
+
If your application defines encrypted properties in your properties file, you can also add an argument such as `-M-Dencryption.key=my-key-value` to the `"mule.runtime.args"` key. See <<test-deploy>> for guidance.
. Launch the `deploy.json` file for your application by pressing Cmd+shift+p (Windows) or Ctrl+shift+p (Mac) and providing
+
[source,command]
----
