...
You can also create separate properties files for your unecrypted and encrypted properties. For example: 
...
[[reference-use]]
=== Use Properties in the Configuration XML

Reference a properties file that contains some encrypted values or a file with no encrypted values, and use the required syntax to use the property value in the configuration XML.

* If your properties file does not contain _any_ encrypted properties:
** Use this syntax: `${property-key}`
...
** Use this syntax: `${secure::property-key}`
