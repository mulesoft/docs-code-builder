...
When developing your application in the IDE, create separate properties files for each of the target environment (such as development, sandbox, or production) that requires different settings, and provide the appropriate settings in each file. The files can include encrypted and unencrypted property values.
