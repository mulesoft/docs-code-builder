...
----
+
Important: Be sure to store your encryption key value (password) in a safe location that you can retrieve when you deploy your application. If you do not enter the correct password, the application is able to upload to CloudHub, but the deployment will fail because the password will not work.
+
.Encrypted Output:
[source,terminal]
----
...
+
To use the variable `$\{env}` in your filename configuration: 
...
If you don't hide the value of an encryption key or other property in the Runtime Manager UI when you deploy an application to CloudHub, Runtime Manager displays that value (see xref:cloudhub-1::cloudhub-manage-props.adoc[]). 
