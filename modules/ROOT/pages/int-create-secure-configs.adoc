...
////
HIDING SINCE IT'S COVERED ELSEWHERE
...
////
HIDING - MOVING PARTS TO CONSOLIDATE INFO
[[prop-files]]
...
Consistent filename patterns enable you to use a `$\{env}` variable in your configuration XML for the prefix to the filename, for example, `$\{env}.properties.yaml` or `$\{env}.secure.yaml`. 
...
Use the Secure Properties Tool to encrypt sensitive data in properties files for your Mule application.

image::acb-project-directory.png["Project Directory for a Mule Application Workspace"]

. Create a properties file with _unencrypted_ properties:
.. Open the `src/main/resources` directory of your project, and create a YAML file with a name like `dev.secure.yaml` or `local.secure.yaml` that configures properties as key-value pairs. For example:
+
[source,yaml]
----
myprop : "somevalue"
...
+
Important: +
Be sure to store your encryption key value (password) in a safe location that you can retrieve when you deploy your application. If you do not enter the correct password, the application is able to upload to CloudHub, but the deployment will fail because the password will not work.
+
.Encrypted Output:
[source,terminal]
----
...
[[use]]
== Use Encrypted Properties in the Configuration XML
...
For reference, examples in the procedure are based on these configurations:
...
<mule xmlns="http://www.mulesoft.org/schema/mule/core" xmlns:doc="http://www.mulesoft.org/schema/mule/documentation"
  ...
  <flow name="my-secure-data-flow" >
    ...
    <logger doc:name="Logger" doc:id="ghijkl" level="WARN" message="#[payload]" category="INSECURE PROPERTY EXAMPLE" />
...
To use encrypted properties: 

. Use an encrypted value in a Mule application:
+
.. Add the key to the encrypted property as a value of an attribute in the connector or component XML. For example:
+
[source,xml]
----
<set-payload 
...
----
.. Above the `<flow/>` element in your the configuration XML, add the Secure Configuration Properties _snippet_ (`<secure-properties:config/>`) by pressing Ctrl+Space (Mac) or Cmd+Space(Windows), typing the initial letters of the element name, such as `secu`, and then selecting the snippet.
+
...
+
If you want to use the variable `$\{env}` in your filename configuration, see <<create-prop-file>> for guidance.
...
If you don't hide the value of an encryption key or other property from the Runtime Manager UI when you deploy an application to CloudHub, Runtime Manager displays that value (see xref:cloudhub-1::cloudhub-manage-props.adoc[]). 
...
* Click *Apply Changes*.
+
Runtime Manager applies your changes and attempt to deploy your application. 
...
+ 
For guidance with naming and configuring the contents of properties files, review <<create-prop-file>>.
