...
Understand configuration properties in Mule applications before following the procedures for encrypting the properties or deploying applications that use the properties. 
...
The following examples use the Blowfish algorithm to encrypt two strings (`"myusername"` and `"mypassword"`), provide the same encryption key _value_ (`my-key`), and use the default encryption mode, CBC. 
...
For more encryption options, see xref:mule-runtime::secure-configuration-properties.adoc[].
...
.. Create a YAML file that configures properties as key-value pairs. For example:
+
[source,yaml]
----
myprop : "somevalue"
...
----
+
Note that you do not add the _value_ of the encryption key yet, only the _name_ (`my-key` in the example). You add a _value_ of your choice for the key before testing a local deployment within your IDE and when deploying an application to CloudHub. 
+
.Encrypted Output:
[source,terminal]
----
...
.. Deploy the application with *Run and Debug* by pressing Ctrl+shift+p (Windows) or CmdCtrl+shift+p (Mac), entering *View: Show Run and Debug*, and clicking *Run and Debug*.
+
...
Hide the name of an encryption key in the Runtime Manager UI when you deploy an application to CloudHub. 

Before you begin: 

* Test encrypted properties used by your Mule application in your IDE. See <<properties>> for guidance.
...
. Provide the _name_ of the encryption key (`encryption.key` by default) that you used to encrypt secure properties to the `mule-artifact.json` file for your application. For example:
+
----
{
...
----
+
The name (not the value) of your encryption key goes into a comma-separated `secureProperties` array. When you deploy your application to CloudHub, the IDE prompts you to provide a _value_ of your choice for the encryption key. 
. Deploy your application to CloudHub by pressing Ctrl+shift+p (Windows) or CmdCtrl+shift+p (Mac) and entering *MuleSoft: Deploy to CloudHub*.
. When prompted, enter _the same value_ you used to encrypt your secure properties.
+
If you do not enter the same value, the application is able to upload to CloudHub, but the deployment will fail.
. Select the *Design* environment for your deployment.
+
Other environments are not supported.
+ 
The IDE indicates that the deployment is in progress and prompts you to open the application in Runtime Manager when the deployment completes successfully/ 
