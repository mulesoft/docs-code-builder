...
. Launch the `deploy.json` file for your application by pressing Cmd+shift+p (Windows) or Ctrl+shift+p (Mac) and entering *MuleSoft: Deploy to CloudHub*.
...
. Click *Settings* > *Properties* for your application to check for the hidden value to the encryption key and for the `env` setting. 
+
You can change the value of the properties, for example, to fix any deployment errors. 
