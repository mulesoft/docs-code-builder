...
== Import a Studio Project into Anypoint Code Builder

//TODO: Verify if this explanations is not duplicated.
...
* Define configurations to a workspace by adding them to the `.code-workspace` file.
...
. In Anypoint Code Builder, go to *File* > *Add Folder to Workspace*.
...
If the Mule runtime or Java version in the project is not compatible with the IDE, it shows a *Mule runtime or Java version requires further action* error.
