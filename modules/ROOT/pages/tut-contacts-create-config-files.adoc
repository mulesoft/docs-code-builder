...
A configuration file is a `.yaml` or `.properties` file that can reference different configurations and specify different values for your connector properties. Configuration files enable you to define specific properties depending on the environment to which you are deploying your application.
...
. In your Explorer view, right-click *src* > *main* > *mule* > *resources* and select *New File*:
+
...
. Configure the `username` and `password` properties to use the values on your configuration file:
+
[source,XML]
--
