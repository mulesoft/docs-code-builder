...
+
Validation times for a ruleset (such as 19.25s) represent the sum of validation times for each rule validation. However, due to parallel execution of rules on machines with multiple cores, a ruleset's validation time is likely to be greater than rule validations take to complete.
+
