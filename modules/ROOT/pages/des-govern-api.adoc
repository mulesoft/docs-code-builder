...
+
Execution times for a ruleset (such as 19.25s) represent the sum of execution periods for each rule validation. Due to parallel execution of rules on machines with multiple cores, the execution time displayed for a ruleset is likely to be greater than the actual execution time.
+
