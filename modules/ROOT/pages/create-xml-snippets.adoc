...
Open the Command Palette (`ctrl/cmd + shift + P`) and type _Snippets: Configure User Snippets_.
. In the *Select Snippets File or Create Snippets* section type _mule-xml_.
+
image::select-snippets.png["User snippet options"]
+
//dm: Consider deleting this image as it's basically the same as the previous one.
