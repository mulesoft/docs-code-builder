= Create XML Snippets for the American Flights Example
:page-deployment-options: cloud-ide, desktop-ide
:page-aliases: tut-dev-create-xml-snippets.adoc
...
// this topic seems to cover more than just creating XML snippets. I think it needs some work.

Before you create an integration project for the American Flights example, define user snippets for the project. Snippets are reusable code patterns that you can insert from the IDE into the configuration XML of Mule applications. 

== Before You Begin

// These prereqs seems to be design rather than integrate. Should this cover creating an integration project?
Design the xref:design-api-specification-from-scratch.adoc[American Flights API] example, completing all the remaining tasks: creating the API specification, adding examples, testing with the mocking service, synchronizing with Design Center, and publishing to Exchange.

== Get Familiar with Snippets
...
The XML generated from the snippets helps you configure your applications more quickly and consistently than with the basic XML elements available from the configuration XML. For example:
...
For more information, see xref:work-with-code-snippets.adoc[].

== Add User Snippets to Your IDE

Define user snippets for Transform (`<ee:transform/>`) and other components in the `mule-xml.json` file. User snippets help you get started with component configurations that are not built in. The snippets you create are available across your Mule projects. You can configure snippets for connector operations, Core processors, and other processors in the XML configuration file for your Mule applications.  
