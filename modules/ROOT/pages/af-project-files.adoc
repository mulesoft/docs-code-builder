...
[[agent-network-yaml-file-elements]]
=== agent-network.yaml File Elements
...
Provide instructions that are specific to this broker. Although not required, including instructions is recommended.

These instructions often focus on business-oriented concerns.
For example, here's instructions for an employee onboarding broker.
...
You don't have to provide instructions like "split the prompt into tasks", or "select the best tool". The broker does that on its own. These instructions are only necessary for describing specific business processes.
