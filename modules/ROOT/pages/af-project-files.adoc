...
All agent network projects also have an exchange.json file. If you're familiar with Anypoint Platform, you've probably seen this file before. This file contains asset metadata that will be available in Anypoint Exchange after you publish your agent network assets.
...
* assetId: Identifies the agent network YAML file.
* Dependencies: Existing assets that this network needs to reference.
* variables: Nested in the metadata section, it defines all the variables whose values shouldn't be hardcoded in the agent network YAML file, but the user needs to be prompted for those values when publishing the agent network.
...
The above snippet represents a link to a workday agent. The *name* attribute doesn't point to the agent's human friendly or logical name (which can change); instead, it references the *assetId* that workday-agent has in Exchange.

The workday-agent asset can either exist already, or can be defined in this same yaml. In either case, the Exchange *assetId* is the key to this reference.

In some cases, we'll need the ref element to point to an asset that's defined in a different business group. For this, the *ref#namespace* attribute can be used to reference the asset's groupId.
...
The brokers section defines the AI assets in your agent network. A broker is an intelligent routing service that coordinates task delegation across specialized agents in your enterprise. The broker name is used as the assetId in exchange.json. The groupId defaults to the associated business group value used in exchange.json.
...
The `spec` configures the broker's internal implementation. You can think of this section as the source code for your broker.
...
- Onboard in Workday: Create the new employee record. Fetch the address using available tools. Don't ask for the address unless you're not sure how to get it. 
...
{
"message": "Workday and Salesforce onboarding completed. Working on next steps...",
     ...
     instructions:
        - |
          ...
          The process for incident management is:
            1. Fetch Salesforce case details: Retrieve the latest critical case details for the customer.
...
links:
  - agent:
      ref:
name: workday-agent	 # Exchange asset ID of the linked agent. By default, the groupId for this asset is the same as that in exchange.json.
namespace:      		 # Optional. If the asset was defined in a different business group, set the corresponding groupId here.
- agent:
ref:
name: badging-agent
...
Assets, IT Stock, Workday, Payroll and Badges agents are what we call leaf agents. These aren't brokers but preexisting agents whose purpose isn't to coordinate work, but to act.
...
By default, the broker has access to all the tools available in the MCP server. However, the most modern LLMs can only handle around 20 to 25 tools per context before starting to hallucinate. It's good practice to limit the available tools to the minimum needed. You can apply that filtering through the allowed list.
...
| maxConsecutiveErrors | The maximum number of errors that the broker will attempt to recover from before returning a failed status. | Integer | 3
|===
...
----
agents:
workday-agent:			# Used as the assetId of the asset published to Anypoint Exchange. The default groupId is the associated value used in exchange.json.
metadata:
       platform: azure
       protocol: a2a
  badging-agent:
...
Configure the LLMs the brokers use for reasoning. For supported LLMs, see link:https://docs.mulesoft.com/agent-fabric/[Agent Fabric Overview].
...
connections:
  workday-agent:
    kind: agent
    ref:
name: workday-agent		 # Use the same name as in the associated entry in the agents section or use the Exchange asset ID.
    spec:
       url: https://workday-a2a-agent-inkbh1.paga8m.usa-w1.cloudhub.io/workday-agent/
       authentication:
  ...
  salesforce-agent:
    ...
    ref:
      name: salesforce-agent
    spec:
      url: https://salesforce-a2a-agent-inkbh1.paga8m.usa-w1.cloudhub.io/sfdc-agent/
      authentication:
        kind: oauth2-client-credentials
        token:
          bodyEncoding: form
...
Authentication is always expressed in accordance with link:https://swagger.io/docs/specification/v3_0/authentication/[Open API authentication methods]. For supported authentication types, see link:https://docs.google.com/document/d/1dVReyn8BzGdQ9QUF0PU1-0qBKUp7G8wZFZ8sFZs0LFM/edit?tab=t.0#heading=h.z4zzg6shhskh[Considerations: Authentication Types].
