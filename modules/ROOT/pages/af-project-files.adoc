...
|`additionalInterfaces` | A list of additional supported interfaces (transport and URL combinations) | Array | Array of interface objects (see AdditionalInterfaces properties below) | No
|`iconUrl` | An optional URL to an icon for the agent | String (URI) | Valid URI string | No
|`provider` | Information about the agent's service provider | Object | Object with provider properties (see Provider properties below) | No
...
|`capabilities` | A declaration of optional capabilities supported by the agent | Object | Object with capability properties (see Capabilities properties below) | No
...
|`skills` | The set of skills, or distinct capabilities, that the agent can perform | Array | Array of skill objects (see Skills properties below) | No
...
|`extensions` |List of extension capabilities supported by the agent |Array |Array of extension objects (see Extensions properties below) |No
|===
...
|`llm` |LLM configuration for the broker. |Object |See LLM properties below |Yes |-
...
|`links` |Defines the list of agents this agent is connected with. |Array |Array of link objects (see Links properties below) |Yes |-
...
|`taskTimeoutSecs` |A timeout (in seconds) for how long should each orchestration task. |Integer |Positive integer |No |60
|`tools` |The tools available to this broker. |Array |Array of tool objects (see Tools properties below) |No |-
|`policies` |Policy bindings to be applied. |Array |Array of PolicyBinding objects (see PolicyBinding properties below) |No |-
|===
...
|`ref` |The reference to the LLM to be used by the orchestrator. |String |Reference to `references.json#/definitions/LLMRef` |Yes
|`connection` |Connection configuration for the LLM. |Object |Object with connection properties (see LLM.connection properties below) |No
|`configuration` |LLM configuration settings. |Object |Object with configuration properties (see LLM.configuration properties below) |Yes
|===
...
|`ref` |Connection reference. |String |Reference to `references.json#/definitions/ConnectionRef` |Yes
|===
...
|`model` |The name of the model to use. |String |Model name string |Yes
|`openAI` |OpenAI specific configuration settings. |Object |Object with OpenAI settings (see OpenAI properties below) |No
|===

`openAI` has these properties.

[cols="1,2,1,1",options="header"]
|===
|Parameter |Description |Type |Valid Values

|`reasoningEffort` |Constrains effort on reasoning for reasoning models. gtp-5.1 defaults to NONE, previous ones default to MEDIUM. |String |`NONE`, `MINIMAL`, `LOW`, `MEDIUM`, `HIGH`
|`temperature` |Controls randomness in the output. |Number |Any number
|`topP` |Nucleus sampling parameter. |Number |Any number
|`topLogprobs` |Number of most likely tokens to return at each position. |Integer |Any integer
|`maxOutputTokens` |Maximum number of tokens to generate. |Integer |Any integer
|===
...
|`mcp` |MCP server configuration. |Object |Object with MCP properties (see Tools[].mcp properties below) |Yes
|===

===== `tools[].mcp` Properties

[cols="1,2,1,1,1",options="header"]
|===
|Parameter |Description |Type |Valid Values |Required

|`ref` |The reference to the MCP server to be available to this broker. |String |Reference to `references.json#/definitions/MCPRef` |Yes
|`connection` |The connection to be used by this mcp. |Object |Object with connection properties (see Tools[].mcp.connection properties below) |No
|`allowed` |Filters the list of tools advertised by the MCP server to only those in this list. Mutually exclusive with 'denied'. |Array of strings |Array of tool name strings |No
|===

`tools[].mcp.connection` has these properties.
...
|`agent` |Agent connection configuration. |Object |Object with agent properties (see Links[].agent properties below) |Yes
|===

===== `links[].agent` Properties

[cols="1,2,1,1,1",options="header"]
|===
|Parameter |Description |Type |Valid Values |Required

|`ref` |Agent reference. |String |Reference to `references.json#/definitions/AgentRef` |Yes
|`connection` |The connection to be used by this agent. |Object |Object with connection properties (see Links[].agent.connection properties below) |No
|===

`links[].agent.connection` has these properties.
...
|`ref` |Policy reference. |String |Reference to `references.json#/definitions/PolicyRef` |Yes
...
spec:
  ...
  taskTimeoutSecs: 30             # Max number of seconds for each orchestration
...
|`metadata` |LLM metadata configuration. |Object |Either WellKnownLLM or CustomLLM object (see metadata properties below) |Yes
|===
...
|`models` |List of available models for this platform. |Array of strings |Array of model name strings |No
|`transcoder` |Transcoder configuration for custom LLM. |Object |Object with transcoder properties (see CustomLLM.transcoder properties below) |Yes
|===

The `CustomLLM.transcoder` object has these properties.

[cols="1,2,1,2,1"]
|===
|Parameter |Description |Type |Valid Values |Required

|`ref` |Reference to the transcoder policy. |String |Reference to `references.json#/definitions/PolicyRef` |No
|===
...
|`links` |Defines the list of agents this agent is connected with. This defines the agent network and enables inter-agent communication and orchestration.|Array|Array of link objects (see Links array item properties below)|Yes
|===
...
|===
|Parameter |Description |Type |Valid Values |Required
|`ref` |Reference to the agent asset.|Object|Object with ref properties (see Links[].agent.ref properties below)|Yes
|`connection` |The connection to be used by this agent.|Object|Object with connection properties (see Links[].agent.connection properties below)|No
|===

`links[].agent.ref` has these properties.

[cols="1,2,1,2,1",options="header"]
|===
|Parameter |Description |Type |Valid Values |Required
|`name` |Exchange asset ID of the linked agent. This references the `assetId` that the agent has in Anypoint Exchange, not the human-friendly name.|String|Asset ID string (e.g., "hr-agent", "crm-agent"). By default, the groupId for this asset is the same as that in exchange.json|Yes
|`namespace` |Business group ID (groupId) where the asset is defined. Use this when the asset was defined in a different business group than the current one.|String|Group ID string. To find the groupId, look in the `dependencies` section in `exchange.json`|No
|===
...
|===
|Parameter|Description|Type|Valid Values|Required
|`ref` |Connection reference.|String|Reference to `references.json#/definitions/ConnectionRef`. This references a connection defined in the `connections` section|Yes
|===
...
Use these attributes to handle error scenarios.
...
| taskTimeoutSecs | The maximum number of seconds for each orchestration. | Integer | 60
