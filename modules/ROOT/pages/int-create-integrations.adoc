...
+
The runtime dropdown shows only runtimes that are installed on your local disk, including bundled runtimes that are available immediately after installation. By default, the latest bundled runtime (Java 17 compatible) is selected. For Java 8 projects, select the Mule 4.8 runtime (Java 8 compatible, reduced runtime).
+
