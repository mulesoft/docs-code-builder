...
|===
. Create a project from scratch, or start with existing project content from Anypoint Exchange.

* *Empty Project*
+
Start an integration project from scratch.
...
. If you opt to import from Exchange, search for the template or example, mouse over the asset that you want to add, and click *Add Asset*. 
//+
...
Use the canvas UI to add components to your project. This example triggers a flow with an HTTP listener from Anypoint Connector for HTTP (HTTP Connector) and adds a Set Payload and Logger component. When you add connector operations from the canvas UI, Anypoint Code Builder also adds a dependency for the connector to the `pom.xml` and a namespace for the connector to the configuration XML.
