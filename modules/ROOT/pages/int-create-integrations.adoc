...
<flow name="getFlights" >
  <http:listener path="path" config-ref="config-ref" doc:name="Listener" doc:id="rrjiqa" />
  <set-payload value="" doc:name="Set payload" doc:id="gecykt" />
...
To fix the error, configure the attributes properly. For example, to correct the mismatched `config-ref` value for the HTTP listener, select the reference name from the drop-down menu in the component's configuration UI, or type the correct attribute value directly within the XML. For more information about debugging, see xref:debugging-mule-apps.adoc[].
