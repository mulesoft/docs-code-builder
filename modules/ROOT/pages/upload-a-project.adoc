...
Import a sharable JAR file for a snapshot of a Mule project into the desktop IDE. Shareable JAR files are generated through the process described in xref:package-mule-application.adoc#sharable[Package a Project in a Sharable JAR File]. The contents are lightweight because they lack dependencies. The IDE generates the dependencies configured in the `pom.xml` for the file when preparing a UI canvas for your project. 
