...
+
* `com.mulesoft.modules.agent.broker` (Agent broker)
* `INSECURE-LOGGING` (For LLM reasoning, and inputs and outputs from agents and assets defined in `tools` and `links`)

[IMPORTANT]
`INSECURE-LOGGING` is very likely to contain personally identifiable information (PII). Make sure to disable it after troubleshooting is completed.
...
Message loging uses the xref:gateway::policies-included-message-logging.adoc[Flex Gateway Message Logging policy]. In the `connection` section of the agent or MCP server instance you want to log, add the following logging policy inside `policies`:
