...
. In Anypoint Platform, navigate to https://anypoint.mulesoft.com/exchange/?show=org&sort=last[Anypoint Exchange^].
...
. In Anypoint Code Builder, open the Command Palette by pressing Ctrl+Shift+p (Windows) or Cmd+Shift+p (Mac), and provide this command:
+
[source,command]
----
