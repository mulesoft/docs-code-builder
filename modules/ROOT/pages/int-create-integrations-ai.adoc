...
The Generative Flow feature transforms your natural language prompt, which describes your business logic, into a flow that contains the connector operations and processors, in the order needed, to accomplish the specified task. 

For example, if you want to create a flow that reads contacts from Salesforce in JSON format, you can enter a prompt describing that task. The prompt is then transformed into a series of connector operations and processors that are needed to execute that task, in this case: Listener, Query, Transformer, and Logger. A flow is generated, which you can then further customize to suit your needs. 
...
. Click *Open Einstein* in the *Start with AI* card to use the generative flow feature.
. Click *Build a flow* in the *Start From Scratch* card to begin building your flow from scratch.
...
+
Highlight the key MuleSoft components and connector operations to include in the flow, for example, HTTP Connector *Listener* and Email Connector *Send*.
...
* Due to the sensitivity of LLMs, the same prompt might produce different responses, and sometimes, a similar prompt can produce very different responses. 
====
