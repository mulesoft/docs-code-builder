...
* Core processors (also called Core components or Mule components) are programmatic building blocks that provide the business logic for integration applications. Examples include For Each (`<foreach/>`), Transform Message (`<ee:transform/>`), and Flow (`<flow/>`). See xref:acb-reference.adoc[].
