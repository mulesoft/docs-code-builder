...
Get the Anypoint Code Builder experience through the Anypoint Extension Pack for Visual Studio Code. The web-based extensions are preinstalled into a code-server implementation that makes VS Code accessible from a browser. The desktop extensions require installation to your VS Code IDE. See xref:setup.adoc[].
...
+
A subflow is similar to a flow but does not support the configuration of triggers (components that are sometimes called message sources), which are exclusive to flows. 
