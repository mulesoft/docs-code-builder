= Adding Components to Debug Your Application
...
// TODO: Package Explorer, Project Explorer, or Explorer view? "Explorer" or "EXPLORER"?
. Add a new line after your `</db:select>` element and indent it to the same level.
. Start typing `_set-variable_` and select `set-variable` (`mule:set-variable`):
// TODO: If you don't write the dash (-), there is no option to add `set-variable` (`mule:set-variable`). Adding "`set-variable` (`mule:set-variable`)" to avoid misunderstandings with "`muledx:set-variable`".
// TODO: If we are to make tutorials clearer, we may need to add the purpose of adding `set-variable` (`mule:set-variable`).
+
[source,xml]
--
...
// TODO: Depending on the scree resolution, you need to scroll the window to discover the call-out number.
+
value:: `_#[payload]_`.
doc:name:: `_Set Database Payload_`.
variableName:: `_databasePayload_`.
. Create a new line before your `</flow>` element and indent to the same level.
. Start typing `_logger_` and select `logger` (`mule::Logger`):
// TODO: Adding "`logger` (`mule::Logger`)" to avoid misunderstandings with "`muledx:logger`".
// TODO: If we are to make tutorials clearer, we may need to add the purpose of adding `logger` (`mule::Logger`).
+
[source,XML]
--
<flow name="getFlights">
