...
.. *Continue*: Proceeds to the next component with a breakpoint. 
+
Key command: F5
.. *Step Over*: Proceeds to the next component in the flow unless the line contains a Flow Ref component (`<flow-ref/>`). If a Flow ref component is on the line, the debugger does not examine components in the referenced flow. For example:
+
[source,xml]
----
<flow name="helloWorld" doc:name="Flow" >
  <http:listener path="greet" config-ref="HTTP_Listener_config" doc:name="Listener"/>
  <flow-ref name="myNewFlow" /> <!--<1>-->
  <logger doc:name="Logger" message="test1"/> <!--<2>-->
</flow>

<flow name="myNewFlow" doc:name="Flow" >
  <logger doc:name="Logger" message="test1"/>
</flow>
----
[calloutlist]
.. Skip <flow-ref/> logic.
...
+
Key command: F10
.. *Step Into*: If the line contains a Flow ref component (`<flow-ref/>`), clicking *Step Into* executes the Flow Ref logic and proceeds to the referenced flow before returning to any components after the Flow Ref.
For example:
+
[source,xml]
----
...
<flow name="myNewFlow" doc:name="Flow" >
  <logger doc:name="Logger" message="test1"/> <!--<3>-->
</flow>
----
[calloutlist]
.. Skip <flow-ref/> logic.
...
*Step Into* proceeds to the next component for debugging. If the line contains a Flow Ref component, *Step Into* enters the flow referenced.
+
Key command: F11
.. *Step Out*: The debugger exits the current flow and attempts to return to a previous flow. If a previous flow doesn’t exist, the debugger behaves like *Continue*.
+
