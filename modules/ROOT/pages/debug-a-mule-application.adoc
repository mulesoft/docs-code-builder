...
You can debug your Mule application using the embedded debugger in Anypoint Code Builder.+
...
NOTE: Anypoint Code Builder does not have a hot re-deploy mechanism. To re-deploy a running application that you are debugging, open the Command Palette (`ctrl/cmd + shift + P`) and type _MuleSoft: Rebuild Application_ .
...
. *Start Debugging*: Starts the build and run your Mule application in debug mode.
...
In the following section you learn how to set up components that help you debug your Mule application, set up breakpoints for your code, and run a debug session.
