...
:page-aliases: ui-tut-design.adoc, tut-design-api-specification-from-scratch.adoc, design-api-specification.adoc
...
. Design, test, synchronize, and publish the specification by completing these tasks:

.. <<start-spec, Starting American Flights API>>
.. <<add-flight-resource, Adding a RAML Resource and Methods>>
.. <<add-query-params-nested-resources, Adding Query Parameters and Nested Resources>>
...
After creating, testing, synchronizing, and publishing the American Flights API specification, you can xref:tut-implement-am-flights-api.adoc[implement] the specification within an xref:tut-integrate-am-flights.adoc[integration].
...
* Have some familiarity with xref:access-management::business-groups.adoc[business groups]. API specifications belong to a business group.
...
+
If you receive the error *Mule DX API Component was not installed*, wait for Mule runtime to load and for background processes to complete. To monitor background processes, see xref:troubleshoot-loading-issues.adoc[].
...
| *Business Group* | Group in Anypoint Platform for your API specification (see xref:access-management::business-groups.adoc[])
|===
