= Creating a Working API Specification
...
Create a basic RAML API specification using Anypoint Code Builder. Reuse API fragments from Anypoint Exchange, create data types and response examples, test your API using API Console, save your API to Design Center, and publish the API as an asset to Exchange.

After completing the specification, you can follow subsequent tutorials to implement the specification and use it in an integration.

== Start the API Specification

Start creating an API specification from the *Quick Actions* menu in Anypoint Code Builder.

. In the activity bar, click the MuleSoft icon to navigate to Anypoint Code Builder.
. Click *Design an API* from the Anypoint Code Builder menu to start the specification:
...
+
images::acb-api-project.png["Project for American Airlines API"]
...
+
images::acb-api-console.png["A /flights specification with get and post methods"]
+
...
+
images::sub-methods-api-console.png["API Console with nested resource"]
...
== Add Examples to the Specification
...
/flights:
  get:
    queryParameters: //for selecting a destinatin
      destination:
        required: false
        enum:
          - SFO
