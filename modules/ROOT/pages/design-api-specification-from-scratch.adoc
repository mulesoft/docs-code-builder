...
Add a nested resource (`/{ID}:`) to use as a dynamic selector, and specify query parameters (`queryParameters:`) as a set of `enum` values to use for requesting the destination of the flight. 
...
+
This process add the `/exchange_modules` directory in your project with `AmericanFlightDataType.raml`. When the process is complete, the status bar displays the message *Dependencies successfully added*. 
. From the Command Palette, add the fragment, *Training: American Flights Example*:
+
[source,command]
----
MuleSoft: Add fragment dependency from Exchange
...
+
This process adds the fragment `AmericanFlightsExample.raml` to the `/exchange_modules` directory of your project.
