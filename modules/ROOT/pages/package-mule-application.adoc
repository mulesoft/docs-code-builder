...
Use the command `MuleSoft: Export Mule Application` to generate a sharable, lightweight JAR file that you or your teammates can import and continue developing in Anypoint Studio or Anypoint Code Builder. This command works the same way as command sequence `Task: Run Task` > `mule` > `mule: Package Only Sources`.
