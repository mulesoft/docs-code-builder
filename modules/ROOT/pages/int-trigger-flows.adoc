...
----
+
Notice `/api/end-point` in the example. For this case, the endpoint is defined within the `name` attribute of a `<flow/>` element in the interface, and `api` is defined in an HTTP Listener in the interface. 
+
...
----
+
The URL adds `/api` before the `/flights` endpoint to make the request through the `<http:listener/>` in the interface. Generally, a listener is located above the router, as in this example:
+
[source,xml]
----
...
----
+
Use `localhost` instead of `0.0.0.0`, and add the listener `path` for before the router (such as `api`) and the endpoint (such as `mypath2`) to the URL. Get <<trigger-url-interface, more information>> about triggering the endpoint.
