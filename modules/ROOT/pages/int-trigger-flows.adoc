...
----
+
Notice `/api/endpoint` in the example. For this case, the endpoint is defined within the `name` attribute of a `<flow/>` element in the interface, and `api` is defined in an HTTP Listener in the interface. 
+
