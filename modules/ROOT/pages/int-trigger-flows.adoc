...
----
+
It is also important to notice that the URL adds `/api` before the `/flights` endpoint to make the request through the `<http:listener/>` in the interface. Generally, a listener is located above the router, as in this example:
+
[source,xml]
----
