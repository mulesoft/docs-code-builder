...
Deploy a project for your implementation or integration to CloudHub. After deploying to CloudHub, manage your application through Anypoint Runtime Manager.

If you are deploying a project that implements the xref:design-api-specification-from-scratch.adoc[American Flights API example], use this procedure for guidance.
...
+
For example, the name of the American Flights example on the EU cloud is similar to `+american-ws-anypoint-code-builder.eu-s1.cloudhub.io+`. 
+
To start the example, open your REST client or browser, and make a `GET` request to the URI. For example: `+http://american-ws-anypoint-code-builder.eu-s1.cloudhub.io/api/flights+`
