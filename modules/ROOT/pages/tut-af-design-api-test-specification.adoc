...
. In Anypoint Code Builder, open the file for your American Flights API specification (`american-flights-api`).
...
//DONE (hn) TODO duke: screenshot bad res per crispy 
. Click *Send*.
. Verify that the API Console returns the response you defined with the imported fragment (`AmericanFlightsExample.raml` or `AmericanFlightsExample.yaml`).
+
