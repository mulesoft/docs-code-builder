...
When an integration that you are developing builds successfully, the application deploys _locally_, within the IDE. It is common to configure an HTTP Listener to trigger the execution of a deployed application through a URL.
...
* In the web IDE, the local deployment within the IDE is hosted remotely, so you must configure a URL to a proxy for your application's external host. 
+
