...
When you deploy an application to CloudHub, Anypoint Code Builder configures a default `deploy.json` or `deloy_ch2.json` file in your `src/main/resources` directory.
...
. When prompted, select the environment to deploy your application.
+
...
. After you click *Deploy*, the packaging and deployment of the application starts, and the VSCode status bar shows the progress of the deployment.
+
