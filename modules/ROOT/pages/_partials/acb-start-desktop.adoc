...
+
NOTE::
If an Anypoint Platform session is active in any of your browsers when you install Anypoint Extension Pack, the extension pack uses that session to log you in to Anypoint Platform from the IDE. You can xref:anypoint-code-builder::start-acb.adoc#logout-status-bar[log out] or  xref:anypoint-code-builder::start-acb.adoc#logout-status-bar[log in] to the IDE from the status bar. 
