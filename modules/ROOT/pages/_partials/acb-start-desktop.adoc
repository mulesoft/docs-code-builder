...
. Recommended: Ensure that the Anypoint extensions update automatically by right-clicking each extension, determining whether *Auto Update All (From Publisher)* is checked, and if not, selecting *Auto Update* for each extension, for example:
+
