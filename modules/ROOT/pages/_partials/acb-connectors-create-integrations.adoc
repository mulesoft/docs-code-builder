...
+
image::build-flow.png["Canvas with starting card with the options to start from scratch, build a flow, or build a subflow"]
+
. Change the name of the flow: 
.. Click *Flow*. 
.. Click the edit icon to change the name. 
+
image::change-flow-name.png["Arrow pointing to the flow name edit pencil icon"]
+
