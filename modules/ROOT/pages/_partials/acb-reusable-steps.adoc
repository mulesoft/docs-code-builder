...
Anypoint Code Builder supports scaffolding these schema fragment files if referenced from API specs:

[cols="1,1,1,1,1,1"]
|===
| | Avro | JSON | OAS | RAML | YAML
| AsyncAPI | Yes | Yes | No | No | No
| REST | No | No | Yes | Yes | No
|===

If you add a fragment that isn't supported, the scaffolder doesn't add these fragments as project dependencies if you import the spec from Exchange. However, you can scaffold schemas that are specified inline within the spec.
