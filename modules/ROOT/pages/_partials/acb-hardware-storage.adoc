...
* [ ] macOS and Linux: `java` — same as on Windows, using the resolved path from your JDK install
* [ ] Windows: `node.exe` — Node.js used by VS Code extensions and scripts
* [ ] macOS and Linux: `node` — same as on Windows
* [ ] Windows: `Code.exe` — Visual Studio Code
* [ ] macOS: Visual Studio Code application bundle — for example, `Visual Studio Code.app`
* [ ] Linux: VS Code binary — for example, the `code` executable from your package or archive install

*Directories*

Unless noted, paths for macOS also apply on Linux.

* [ ] User profile Anypoint Code Builder data: +
Windows: `%USERPROFILE%\AnypointCodeBuilder` +
macOS and Linux: `~/AnypointCodeBuilder`
* [ ] VS Code user data: +
Windows: `%USERPROFILE%\.vscode` +
macOS and Linux: `~/.vscode`
...
Linux: `/tmp` or the directory in `TMPDIR` (often `/tmp`)
...
* Linux: Avoid CPU governors or profiles that aggressively downclock the CPU during intensive IDE and Maven activity; use a performance-oriented governor if your distribution supports it.
