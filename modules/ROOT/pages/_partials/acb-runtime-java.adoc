...
You can select any of the supported versions. When you create the project, the IDE automatically downloads selected Mule and Java versions that append the message *(downloads upon project creation)* and saves the settings to the project's `mule-artifact.json` file.
...
//TODO: CHECK LOCATION OF DOWNLOADS: ${user.home}/runtimes and ${user.home}/jdk
The IDE provides a notification if it is necessary to download the selected Mule runtime or Java version for the project. Mule runtime downloads to `${user.home}/AnypointCodeBuilder/maven/runtimes`, and the selected Java version downloads to `${user.home}/AnypointCodeBuilder/jdk`.
