...
The payload can be a literal string or a DataWeave expression. Use the `set-payload` component for simple expressions or transformation. 
...
[WARNING]
--
The `mimeType` and `encoding` attributes do not affect a DataWeave expression used as `value`. They only affect the output Mule Message. If a transformation is required, the DataWeave expression must contain an explicit output directive.
--
