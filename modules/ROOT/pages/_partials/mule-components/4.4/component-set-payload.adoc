...
Set Payload (`<set-payload/>`) provides attributes in the configuration UI and XML:
...
| N/A
| `resource`
|  Points to a `.dwl` file containing a DataWeave transformation script. These values are resolved statically (not dynamically). For example, using `<ee:set-payload resource="myscript-${env}.dwl"/>` generates an error. For more information, see xref:dataweave::dataweave-language-introduction.adoc#dwl_file[dwl File].
...
//entire Properties row in this table
