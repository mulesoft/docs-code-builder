...
Ensures that only unique messages continue through a flow execution by checking the unique ID of the incoming message.
...
The following examples show how to configure an Idempotent Message Validator to extract an ID from an HTTP request to use as a the unique identifier and how to hash the payload using a DataWeave expression and use the message ID as the unique identifier. 
...
The following example shows an Idempotent Message Validator configured to hash the payload using a DataWeave expression, store the result in a persistent object store, and then use the processed message IDs as the unique identifier to filter the message:
