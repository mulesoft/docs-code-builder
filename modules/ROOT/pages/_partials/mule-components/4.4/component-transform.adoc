...
| *Set payload*
| `<ee:set-payload/>` 
| Sets or transforms the xref:mule-runtime::about-mule-message.adoc#message-payload[Mule message payload]. This child element of `<ee:message/>` accepts a DataWeave script. One `<ee:set-payload/>` element is allowed. 
...
| *Set Variables*
| `<ee:set-variable/>` 
| Sets or transforms a xref:mule-runtime::about-mule-variables.adoc[Mule variable]. This child element of `<ee:variables/>` accepts a DataWeave script. One or more `<ee:set-variable/>` elements are allowed.
|===
...
+
For more information about `attributes` and other Mule variables, see xref:dataweave::dataweave-variables-context.adoc[].
...
+
All the metadata is selectable. For example, you can select the reason phrase with `attributes.reasonPhrase` and the headers with `attributes.headers`. For more information, see xref:dataweave::dataweave-selectors.adoc[].
...
* xref:anypoint-code-builder::component-set-payload.adoc[]
* xref:anypoint-code-builder::component-set-variable.adoc[]
