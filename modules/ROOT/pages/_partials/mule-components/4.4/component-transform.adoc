...
| `<ee:set-payload/>` | Sets or transforms the xref:4.4@mule-runtime::about-mule-message.adoc#message-payload[Mule message payload]. This child element of `<ee:message/>` accepts a DataWeave script. One `<ee:set-payload/>` element is allowed. 
...
| `<ee:set-variable/>` | Sets or transforms a xref:4.4@mule-runtime::about-mule-variables.adoc[Mule variable]. This child element of `<ee:variables/>` accepts a DataWeave script. One or more `<ee:set-variable/>` elements are allowed.
|===
