...
+
You can provide format-specific resources and settings for xref:dataweave::dataweave-formats.adoc[supported MIME types (formats)].
--
...
This example calls an external DataWeave script within a xref:dataweave::dataweave-language-introduction.adoc#dwl_file[DWL file] to set the payload. 
...
For demonstration purposes, this example triggers a flow that sets a payload and variables and then uses `<ee:transform/>` to select the value of the payload object, attributes returned as HTTP metadata from the listener, and the entire content of the Mule variable. The loggers in the example help track the values within the Mule event in the flow.
