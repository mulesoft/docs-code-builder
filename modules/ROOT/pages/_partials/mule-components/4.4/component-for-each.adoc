...
If one of the elements in a collection throws an exception, For Each stops processing that collection and invokes the error handler. Unlike Parallel For Each, which processes all routes even when errors occur, For Each processes elements sequentially and halts execution when an error is encountered.

You can add an xref:acb-component-error-handler.adoc[Error Handler] component to handle errors that occur during processing of the collection elements.
