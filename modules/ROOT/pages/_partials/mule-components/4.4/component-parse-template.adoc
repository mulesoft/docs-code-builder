...
A template is defined as text with embedded DataWeave expressions that Mule evaluates and replaces with their result.

You can configure a template through an external file reference or you can embed the template in the component definition.
...
| `location` 
| Location of the file that Mule uses as a template into which to insert values extracted from the message properties or variables.

| `content`
| String representing the template to be used where the embedded expressions will be evaluated and replaced by their results.

| `target`
| Name of the variable where the result of the expression defined in `targetValue` is stored after the Parse Template is executed.
