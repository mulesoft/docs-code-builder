...
If there's no error handling configuration that matches the error, the app follows a default error handling process. See xref:mule-runtime::error-handling.adoc#default_error_handling[Using Default Error Handling for Messages] for more information.
...
* xref:mule-runtime::mule-server-notifications.adoc[Mule Runtime Engine Notifications]
