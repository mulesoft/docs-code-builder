...
* On Error Propagate (`on-error-propagate`): rolls back any transactions, executes, and uses that result to re-throw the existing error. In this case, the container element "fails" its execution.

If there's no error handling configuration that matches the error, the app follows a default error handling process. See xref:mule-runtime::error-handling.adoc#default_error_handling[] for more information.
...
| `enableNotifications` | Specifies if this strategy fires a notification of type `ExceptionNotification` when an error occurs. Defaults to `true`.
...
* xref:mule-runtime::mule-server-notifications[Mule Runtime Engine Notifications]
