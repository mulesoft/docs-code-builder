...
Use Anypoint Code Builder to start an API implementation project by importing the API spec and scaffolding it into an interface for your implementation and integration projects (Mule apps).
...
After scaffolding an API specification into an interface, you are ready to develop, test, and deploy your Mule application as described in xref:int-developing-integrations.adoc[]. To understand how an integration project can implement an API specification, see the tutorial xref:tut-af-implement-am-flights-api.adoc[].
