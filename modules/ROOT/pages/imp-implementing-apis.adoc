...
Use Anypoint Code Builder to implement an API by importing the API spec and scaffolding it into an interface for your implementation and integration projects (Mule apps).

Anypoint Code Builder supports implementing OAS, RAML, and GraphQL APIs,
except for APIs that use OAS or JSON schema fragment dependencies from Exchange.
...
After scaffolding an API specification, you are ready to develop your Mule application as described in xref:int-developing-integrations.adoc[]. To understand how an integration project can implement an API specification, see the tutorial xref:tut-af-implement-am-flights-api.adoc[].
