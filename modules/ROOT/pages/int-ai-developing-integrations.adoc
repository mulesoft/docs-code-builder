= Developing Integrations Using Agentforce
:page-deployment-options: cloud-ide, desktop-ide

To jumpstart your integration development, use natural language input in Agentforce for Anypoint Code Builder Generative Flows to generate flows for you. The engine for these generative flows is the pre-trained large language models (LLM) that exist within the Salesforce Shared Trust Boundary. These LLMs are at the core of the Generative Flows capabilities in Anypoint Code Builder.

NOTE: Agentforce for Anypoint Code Builder Generative Flows is available only on US and EU cloud hosts. Canada and Japan cloud hosts are currently not supported.

== Trust Layer

The Agentforce trust layer bridges Anypoint Platform and the LLMs, as shown in the following flow: 
