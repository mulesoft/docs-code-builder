...
Use natural language prompts in Einstein for Anypoint Code Builder Generative Flows to jumpstart your integration development by generating flows for you. The engine for these generative flows is the pre-trained large language models (LLM) that exist within the Salesforce Shared Trust Boundary. These LLMs are at the core of the Generative Flows capabilities in Anypoint Code Builder. 
...
. MuleSoft validates the generated output to make sure that the generated flow works within the MuleSoft ecosystem. 
...
* https://help.salesforce.com/s/articleView?id=sf.generative_ai_trust_overview.htm&type=5[]
