...
. Right-click the component. To copy multiple components, press and hold the Ctrl key (Windows) or Cmd key (macOS), select the components, and then right-click on a component.
. Select *Copy Component*. Or press Ctrl+c (Windows) or Cmd+c (macOS) to copy the components.
...
[NOTE]
Some components, like an HTTP Listener, can't be pasted to certain locations. An error message shows if you try to paste a component to an invalid location.
