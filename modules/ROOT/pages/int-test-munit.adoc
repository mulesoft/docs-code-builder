...
+
A canvas appears with a link to build a new test with scope and flow structure options.
+
...
Create an MUnit test in a new or existing test suite. Use the canvas, the XML Editor, or right-click the `flow` component to start an MUnit test. For guidance with your test configuration, refer to the <<prereqs, MUnit documentation>> described in the prerequisites.
