...
With MUnit, create MUnit tests and MUnit test suites. MUnit test is an individual test case, defined by the `<munit:test>` element, which includes specific validations and assertions to make sure the functionality of a Mule application. A MUnit test suite is a container for multiple MUnit tests, organizing them within an XML file typically found in the `/src/test/munit` directory.
...
For MUnit features supported by Anypoint Code Builder, see xref:acb-release-notes.adoc[Anypoint Code Builder Release Notes]. Anypoint Code Builder - MUnit Extension, part of the Anypoint Extension Pack, supports MUnit functionality in Anypoint Code Builder. For information about extensions, see xref:index.adoc#extension-pack[Anypoint Extension Pack].
...
Configure MUnit tests and test suites using the canvas, the XML editor, or directly from a flow.
...
+
After the February 2025, the IDE creates a canvas with a link to build a new test with scope and flow structure options.
+
...
Create an MUnit test in a new or existing test suite. Use the canvas, the XML Editor, or right-click a flow to start an MUnit test. For guidance with your test configuration, refer to the <<prereqs, MUnit documentation>> described in the prerequisites.

==== Create a Test from a Flow
You can quickly generate the basic structure for an MUnit test designed to test a specific flow within your Mule application.

. Open your Mule project.
. In the canvas, right-click on your flow.
. Select *Create Test*.
...
==== Create a Test from the XML Editor
...
[[observe-processors-in-your-flow]]
== Observe Processors in Your Flow
...
| `Mock-when` | Create a mock of an event processor in your flow to simulate the behavior of the event processor without actually executing it.
...
. Search for a processor designed to observe your flow (see <<observe-processors-in-your-flow>>).
...
+
The processor appears in the XML editor and the canvas.

When selecting the processor and its attributes, consider these aspects:

* The setup configured for one processor applies to all processors of the same type in your flow.
