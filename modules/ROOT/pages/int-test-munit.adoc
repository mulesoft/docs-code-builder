...
Configure your project to connect to MUnit resources. The process differs depending on whether you have a Mule enterprise edition (EE) license.

* <<configure-settings-license-yes>>: If you have a Mule EE license (_not a trial license_), use this procedure to set access to MUnit features. 
+
Configure `server` credentials and `pluginRepository` settings in the Maven file, `settings.xml`. This configuration applies across all implementation and integration projects in Anypoint Code Builder because all settings are in the Maven `settings.xml` file.

* <<configure-settings-license-no>>: If you _don't have_ a Mule EE license or if your license is a trial license, use this procedure to set access to MUnit features. 
+
Configure your `mule-artifact.json` for the project and `pluginRepository` settings in the Maven file, `settings.xml`. Access applies per project because of the project-level settings in `mule-artifact.json`.

[[configure-settings-license-yes]]
=== Configure Maven Settings (Licensed Access)

If you have a Mule EE license (_not a trial license_), configure `server` credentials with `pluginRepository` settings in the Maven file, `settings.xml`.
...
//add plugin repo setting:
. {empty}
include::anypoint-code-builder::partial$acb-maven-settings.adoc[tag=add-plugin-repo-settings]
...
If you do not have a Mule EE license or if your license is a trial license, configure your `mule-artifact.json` for the project and `pluginRepository` settings in the Maven file, `settings.xml`.
