...
+
Configure `server` credentials settings in the Maven file, `settings.xml`. This configuration applies across all implementation and integration projects in Anypoint Code Builder because all settings are in the Maven `settings.xml` file.
...
+
Configure your `mule-artifact.json` for the project in the Maven file, `settings.xml`. Access applies per project because of the project-level settings in `mule-artifact.json`.
...
If you do not have a Mule EE license or if your license is a trial license, configure your `mule-artifact.json` for the project.
