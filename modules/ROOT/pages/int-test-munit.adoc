...
:right-click: You can also right-click to select the same options.
...
For MUnit features supported by Anypoint Code Builder, see xref:acb-release-notes.adoc[]. xref:studio::index.adoc[Anypoint Studio] also supports MUnit testing features.
...
//
For MUnit features supported by Anypoint Code Builder, see xref:acb-release-notes.adoc[]. Anypoint Studio supports testing with the full range of MUnit features.
...
Run your test in debug mode after adding breakpoints to the XML for your tests or Mule application. When Mule reaches the breakpoint, you can review variables in the Run and Debug panel, including test and Mule event variables. 
...
. When the build process reaches a breakpoint, click the image:icon-run-debug.png["",18,18] (Run and Debug panel) icon from the activity bar to review variables, including the Mule event and test parameters. For example:
+
...
----
.. In the `launch.json` file, add a comma after the first object in the array followed by the JSON object named `Munit Debug Tests`. For example:
+
[source,JSON]
----
{
...
You can also add the configuration by pressing Ctrl+Space in `launch.json` to open the auto-complete menu and selecting *MUnit Debug Launch*. For example:
+
...
image:munit-launch-config-munit.png.png["Selecting *MUnit Debug Tests* from the launch menu"]
.. Click the image:icon-start-debug.png["",18,18] (launch) button. 
. Verify that the MUnit tests run. 
