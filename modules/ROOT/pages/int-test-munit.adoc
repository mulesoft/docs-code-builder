...
+
Access to MUnit features applies across all implementation and integration projects in Anypoint Code Builder because all settings are in the Maven `settings.xml` file.

* <<configure-settings-license-no>>:
+
If you _don't have_ a Mule EE license or if your license is a trial license, configure your `mule-artifact.json` for the project.
+
Access to MUnit features applies per project because of the project-level settings in `mule-artifact.json`.
...
----
. {empty}
include::anypoint-code-builder::partial$acb-maven-settings.adoc[pom-releases-ee]
