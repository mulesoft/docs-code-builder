...
* You must have an enterprise license to access MUnit resources through your Maven `settings.xml` file. See <<maven-settings>>.
+
For more information, see xref:mule-runtime::maven-reference.adoc#configure-mule-repositories[MuleSoft Repositories].
...
//MUNIT SETTINGS
[[maven-settings]]
== Provide Server and Plugin Repository Settings 

Configure your Maven `settings.xml` to connect to MUnit resources. For more information about `settings.xml`, see the https://maven.apache.org/settings.html[Maven documentation].

. Navigate to `settings.xml`, typically in `{user.home}/.m2/`.
. Add the following settings within the `<settings></settings>` tags, replacing the values `yourusername` and `yourpassword` with your Anypoint Platform user name and password.
+
[source,xml]
----
<server>
...
----
. Add the following `pluginRepository` setting to the `<pluginRepositories/>` element (within the `<profile/>`):
+
[source,xml]
----
    <pluginRepository>
...
For more information about Maven settings, see xref:mule-runtime::maven-reference.adoc#configure-mule-repositories[].
