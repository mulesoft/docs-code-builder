...
Anypoint Code Builder provides a *Testing* panel from which you can run tests. In the *Explorer*, test suites are also located within the project directory `{munit-src}`. The IDE also shows a canvas for creating and maintaining MUnit test suits, a Testing Results panel with the status of tests, and logs that identify the status of tests. 
...
. The IDE creates an empty test in the currently open test suite.
...
After the February 2025 release, the IDE shows a canvas with the test components, or provides a link to build a new test.   
...
* In the activity bar, click (image:anypoint-code-builder::icon-explorer.png["",18,18]) (*Explorer*). The IDE shows Coverage for directories and files.
