...
Run, modify, and create MUnit suites and tests for your implementation and integration projects in Anypoint Code Builder. Import projects that contain MUnit test files. 
...
Both the Test Results panel and the Terminal display the results of the test. The Test Results panel also provides time-stamped logs of the tests you run, unless you clear the logs.
...
Run tests from the test suite XML files by using one of the following options in the suite's right-click menu.

image:munit-xml-test-options.png["Testing options from test suite XML"]

To run tests from the test suite XML: 

. In your project, open the Testing panel from the activity bar.
+
See TODO_TODO for guidance.
. Open your test suite XML file. 
+
See TODO_TODO for guidance.
//(xmls with <munit:config> under src/test/munit)
. Run a single test or all tests:
.. To run all tests in the test suite XML: 
... Place your cursor anywhere in the XML file.
...
<munit:test name="test-02-suite1">
  <munit:validation>
    <munit-tools:assert-equals actual='#["c"]' expected='#["c"]' />
  </munit:validation>
</munit:test>
---- 
... Right-click, and select one of the following options:
