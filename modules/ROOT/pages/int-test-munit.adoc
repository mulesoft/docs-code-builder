...
Anypoint Code Builder provides a *Testing* panel from which you can run tests. In the *Explorer*, test suites are also located within the project directory `{munit-src}`. The IDE also provides a canvas for creating MUnut test suits, a Testing Results panel that provides the status of tests, and logs that identify the status of tests that you run. 
