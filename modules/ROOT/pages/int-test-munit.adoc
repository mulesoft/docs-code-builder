...
Anypoint Code Builder provides a *Testing* panel from which you can run tests. In the *Explorer*, test suites are also located within the project directory `{munit-src}`. The IDE also provides a canvas for creating and maintaining MUnut test suits, a Testing Results panel that provides the status of tests, and logs that identify the status of tests that you run. 
...
If you don't own a Mule Enterprise Edition (EE) license or if your license is a trial license, configure your `mule-artifact.json` for the project.
...
* With the *Behavior* components, you can set the conditions for the test. 
* With the *Execution* components, you can specify the actions or processes to test.
* With the *Validation* components, you can check the output or state of the application to ensure it matches the expected results so you can confirm the application behaves as intended.
. Continue to <<create-test>>.

[[create-test]]
=== Create an MUnit Test

Create an MUnit test in a new or existing test suite. Use auto-complete from the MUnit XML to start an MUnit test. For guidance with your test configuration, refer to the <<prereqs, MUnit documentation>> described in the prerequisites.
