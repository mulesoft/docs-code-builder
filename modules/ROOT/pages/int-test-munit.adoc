...
----
//// 
MUNIT NOT SUPPORTED IN OCT:
.. In the `launch.json` file, add a comma after the first object in the array, and append the MUnit launch configuration by using xref:start-discover-ui.adoc#auto-complete[auto-complete] (or pressing Ctrl+Space) and selecting *MUnit Debug Launch*. For example:
+
...
. In the XML, use xref:start-discover-ui.adoc#auto-complete[auto-complete] (or press Ctrl+Space) to display available options, and type `munit`. For example:
+
