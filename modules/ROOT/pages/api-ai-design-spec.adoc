...
* *Act Mode*  
Dev Agent attempts to perform the actions outlined in the plan, using the MuleSoft MCP Server tools to modify files, create resources, or execute changes in your project.
...
These pipelines are exposed to Dev Agent through the following MCP tools:
...
You can use these tools to generate API specifications, create and deploy integrations, manage governance rulesets, build Agent Fabric agent networks, work with DataWeave, and gain insights into how your applications and APIs perform—all directly from Anypoint Code Builder.

The following MuleSoft MCP Server tools are available in Anypoint Code Builder and are also supported when you use the Anypoint Extension Pack in compatible IDEs such as VS Code, Cursor, and Windsurf:
...
* `create_MCP_server` – Create a MuleSoft MCP server using the MCP Connector in a Mule project.
* `create_mule_project` – Create a Mule project structure in a given directory.
* `generate_api_spec` – Generate, modify, or enhance an API specification using AI.
...
* `search_asset` – Search for assets in Anypoint Exchange using flexible filters and criteria.
* `test_connection` – Validate connector configurations (such as HTTP or Salesforce connectors) in a Mule project.
* `validate_project` – Validate the current project—such as an API spec or agent network project—before deployment or publishing.
...
* `dataweave_open_project` – Open a DataWeave project from a given URL in the current or a new window.
* `dataweave_run_script_tool` – Run a DataWeave transformation with sample data to validate script output.
...
* `get_flex_gateway_policy_example` – Retrieve Flex Gateway policy examples to help configure Rust-based custom policies.
...
. Select the gear icon to open full MCP Server settings.
...
These views let you install new MCP servers, connect remote MCP servers, or manage existing ones.


=== Add an MCP Server

You can add a custom MCP server in three ways: installing from the Marketplace, adding a remote MCP server, or manually editing the configuration file.
...
Each MCP server provides a set of tools—actions that Dev Agent can perform.
...
You can configure how long Dev Agent waits for a response from an MCP server:

. In the server details panel, locate *Request Timeout*.
. Select a timeout value: 30 seconds, 1 minute, 5 minutes, 10 minutes, 30 minutes, or 1 hour.
...
Tool availability, behavior, and stability may vary depending on the MCP server version, configuration, and whether the server is community-provided or supported by Salesforce.
...
. Select files or folders to attach, paste terminal input/output, or paste a URL.
...
* *Delete specific tasks* – Select individual tasks and click *Delete Specific*.
* *Delete all history* – Select *Delete All History* to clear all tasks.
...
To set custom metadata using sample data:
