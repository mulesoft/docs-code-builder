...
Complete all the procedures in xref:tut-design-api-specification-from-scratch.adoc[].
...
+
To insert the `!include` directive and file manually from the specification file, you can use built-in functionality. For example:
+
* After typing `AmericanFlight: `, click Ctrl+Spacebar, start typing `!include`, and select `!include` from the drop-down menu in the RAML file. For example:
+
