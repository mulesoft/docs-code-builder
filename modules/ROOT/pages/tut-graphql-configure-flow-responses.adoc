...
. Using `curl`, create a `POST` request to the local address and send a `bookById` query. See xref:tut-graphql-test-first-implementation.adoc[Test Your GraphQL Implementation] for examples.
